            ...
            let filtered: Vec<Value> = all_tasks
                ...
                .filter(|t| {
                    if let Some(ref col) = column {
                        if t.get_str("position_column") != Some(col.as_str()) { return false; }
...
mod tests {
    ...
    async fn test_list_tasks_by_column() {
        let (_temp, ctx) = setup().await;
        let r1 = AddTask::new("Todo task").execute(&ctx).await.into_result().unwrap();
        let id1 = r1["id"].as_str().unwrap();
        AddTask::new("Another todo").execute(&ctx).await.into_result().unwrap();

        MoveTask::to_column(id1, "done").execute(&ctx).await.into_result().unwrap();

        let result = ListTasks::new().with_column("todo").execute(&ctx).await.into_result().unwrap();
    ...
    async fn test_list_tasks_excludes_done_by_default() {
        let (_temp, ctx) = setup().await;
        AddTask::new("Still todo").execute(&ctx).await.into_result().unwrap();
        let r2 = AddTask::new("In progress").execute(&ctx).await.into_result().unwrap();
        let r3 = AddTask::new("Finished").execute(&ctx).await.into_result().unwrap();
        ...
        let result = ListTasks::new().execute(&ctx).await.into_result().unwrap();
        assert_eq!(result["count"], 2);
        let titles: Vec<&str> = result["tasks"].as_array().unwrap().iter().map(|t| t["title"].as_str().unwrap()).collect();
    ...
    async fn test_list_tasks_explicit_done_column_returns_done() {
        let (_temp, ctx) = setup().await;
        let r1 = AddTask::new("Finished task").execute(&ctx).await.into_result().unwrap();
        AddTask::new("Open task").execute(&ctx).await.into_result().unwrap();
        MoveTask::to_column(r1["id"].as_str().unwrap(), "done").execute(&ctx).await.into_result().unwrap();

        let result = ListTasks::new().with_column("done").execute(&ctx).await.into_result().unwrap();
        assert_eq!(result["count"], 1);
    ...
    async fn test_list_tasks_filter_by_ready() {
        ...
        let result = ListTasks::new().with_filter("#READY").execute(&ctx).await.into_result().unwrap();
        assert_eq!(result["count"], 1);
        assert_eq!(result["tasks"][0]["title"], "Blocker");

        // #BLOCKED virtual tag matches only blocked tasks
        let result = ListTasks::new().with_filter("#BLOCKED").execute(&ctx).await.into_result().unwrap();
        assert_eq!(result["count"], 1);
    ...
    async fn test_list_tasks_excludes_archived() {
        let (_temp, ctx) = setup().await;
        AddTask::new("Task 1").execute(&ctx).await.into_result().unwrap();
        let r2 = AddTask::new("Task 2 (to archive)").execute(&ctx).await.into_result().unwrap();
        let id2 = r2["id"].as_str().unwrap().to_string();
        AddTask::new("Task 3").execute(&ctx).await.into_result().unwrap();
    ...
    async fn test_list_tasks_filter_by_tag() {
        let (_temp, ctx) = setup().await;
        AddTask::new("Tagged task").with_description("This task has a #bug tag").execute(&ctx).await.into_result().unwrap();
        AddTask::new("Untagged task").execute(&ctx).await.into_result().unwrap();

        let result = ListTasks::new().with_filter("#bug").execute(&ctx).await.into_result().unwrap();
        assert_eq!(result["count"], 1);
    ...
    async fn test_list_tasks_filter_by_assignee() {
        ...
        AddActor::new("bob", "Bob").execute(&ctx).await.into_result().unwrap();

        let r1 = AddTask::new("Alice's task").execute(&ctx).await.into_result().unwrap();
        let r2 = AddTask::new("Bob's task").execute(&ctx).await.into_result().unwrap();
        AddTask::new("Unassigned task").execute(&ctx).await.into_result().unwrap();

        AssignTask::new(r1["id"].as_str().unwrap(), "alice").execute(&ctx).await.into_result().unwrap();
        AssignTask::new(r2["id"].as_str().unwrap(), "bob").execute(&ctx).await.into_result().unwrap();

        let result = ListTasks::new().with_filter("@alice").execute(&ctx).await.into_result().unwrap();
        assert_eq!(result["count"], 1);
    ...
    async fn test_list_tasks_filter_boolean_logic() {
        ...
        let r1 = AddTask::new("Bug by Alice").with_description("#bug").execute(&ctx).await.into_result().unwrap();
        AssignTask::new(r1["id"].as_str().unwrap(), "alice").execute(&ctx).await.into_result().unwrap();

        AddTask::new("Bug unassigned").with_description("#bug").execute(&ctx).await.into_result().unwrap();
        AddTask::new("Feature by Alice").with_description("#feature").execute(&ctx).await.into_result().unwrap();

        let result = ListTasks::new().with_filter("#bug && @alice").execute(&ctx).await.into_result().unwrap();
        assert_eq!(result["count"], 1);
    ...
    async fn test_list_tasks_unarchive_restores() {
        let (_temp, ctx) = setup().await;
        let r1 = AddTask::new("Task A").execute(&ctx).await.into_result().unwrap();
        let id1 = r1["id"].as_str().unwrap().to_string();

        let ectx = ctx.entity_context().await.unwrap();
        ectx.archive("task", &id1).await.unwrap();
        assert_eq!(ListTasks::new().execute(&ctx).await.into_result().unwrap()["count"], 0);

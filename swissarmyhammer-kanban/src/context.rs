...
mod tests {
    ...
    async fn test_open_seeds_defaults() {
        ...
        // Should have all 29 built-in fields
        assert_eq!(fields.all_fields().len(), 29);
...
"#;
        ...
        // Open — should have 29 built-in + 1 custom = 30
        let ctx = KanbanContext::open(&kanban_dir).await.unwrap();
        let fields = ctx.fields().unwrap();
    ...
    async fn test_fields_accessor() {
        ...
        assert_eq!(task_fields.len(), 18); // title, tags, assignees, project, depends_on, progress, body, position_column, position_ordinal, attachments, virtual_tags, filter_tags, due, scheduled, created, updated, started, completed
    }

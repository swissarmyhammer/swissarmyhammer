...
use swissarmyhammer_kanban::{
    board::InitBoard, dispatch::execute_operation, parse::parse_input, task::ListTasks, Execute,
    KanbanContext,
};
...
async fn s01_filter_by_tag() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Bug task", "description": "#bug report"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Feature task", "description": "#feature request"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Docs task", "description": "#docs update"}),
    )
    .await;
...
async fn s02_filter_by_assignee() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add actor", "id": "bob", "name": "Bob", "type": "human"}),
    )
    .await;
    ...
    dispatch(
        &ctx,
        json!({"op": "assign task", "id": r1["id"], "assignee": "alice"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "assign task", "id": r2["id"], "assignee": "bob"}),
    )
    .await;
...
async fn s03_filter_by_ref() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Depends on dep", "depends_on": [dep_id]}),
    )
    .await;
    ...
    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": format!("^{dep_id}")}),
    )
    .await;
    let t = titles(&result);
    // Both the dependency itself (id match) and the task that depends on it should match
    assert!(
        t.contains(&"Dependency".to_string()),
        "ref should match entity's own id"
    );
    assert!(
        t.contains(&"Depends on dep".to_string()),
        "ref should match depends_on"
    );
...
async fn s04_and_operator() {
    ...
    let r1 = dispatch(
        &ctx,
        json!({"op": "add task", "title": "Bug by Alice", "description": "#bug"}),
    )
    .await;
    dispatch(
        &ctx,
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Bug unassigned", "description": "#bug"}),
    )
    .await;

    let r3 = dispatch(
        &ctx,
        json!({"op": "add task", "title": "Feature by Alice", "description": "#feature"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "assign task", "id": r3["id"], "assignee": "alice"}),
    )
    .await;

    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": "#bug && @alice"}),
    )
    .await;
    assert_eq!(result["count"], 1);
...
async fn s05_or_operator() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Bug", "description": "#bug"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Feature", "description": "#feature"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Docs", "description": "#docs"}),
    )
    .await;

    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": "#bug || #feature"}),
    )
    .await;
...
async fn s06_not_operator() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Has done tag", "description": "#done"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "No done tag", "description": "#bug"}),
    )
    .await;
...
async fn s07_grouping() {
    ...
    dispatch(
        &ctx,
        json!({"op": "assign task", "id": r2["id"], "assignee": "alice"}),
    )
    .await;

    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Docs unassigned", "description": "#docs"}),
    )
    .await;

    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": "(#bug || #feature) && @alice"}),
    )
    .await;
    assert_eq!(result["count"], 2);
    let t = titles(&result);
...
async fn s09_virtual_tag_ready() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Blocked", "depends_on": [id1]}),
    )
    .await;
...
async fn s11_virtual_tag_combined_with_real_tag() {
    ...
    let r1 = dispatch(
        &ctx,
        json!({"op": "add task", "title": "Ready bug", "description": "#bug"}),
    )
    .await;
    let _id1 = r1["id"].as_str().unwrap();
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Ready feature", "description": "#feature"}),
    )
    .await;

    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": "#READY && #bug"}),
    )
    .await;
    assert_eq!(result["count"], 1);
...
async fn s12_keyword_operators_lowercase() {
    ...
    let r1 = dispatch(
        &ctx,
        json!({"op": "add task", "title": "Alice urgent", "description": "#urgent"}),
    )
    .await;
    dispatch(
        &ctx,
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Done tagged", "description": "#done"}),
    )
    .await;
    ...
    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": "not #done and @alice or #urgent"}),
    )
    .await;
    let t = titles(&result);
    assert!(
        t.contains(&"Alice urgent".to_string()),
        "matches both @alice and #urgent"
    );
    assert!(
        !t.contains(&"Done tagged".to_string()),
        "#done excluded by not"
    );
}
...
async fn s13_keyword_operators_uppercase() {
    ...
    let r1 = dispatch(
        &ctx,
        json!({"op": "add task", "title": "Alice no-done", "description": "#bug"}),
    )
    .await;
    dispatch(
        &ctx,
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Done task", "description": "#done"}),
    )
    .await;

    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": "NOT #done AND @alice"}),
    )
    .await;
    assert_eq!(result["count"], 1);
...
async fn s16_no_matches_returns_empty() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "T1", "description": "#bug"}),
    )
    .await;

    let result = dispatch(
        &ctx,
        json!({"op": "list tasks", "filter": "#nonexistent-tag"}),
    )
    .await;
...
async fn s17_tag_names_with_special_chars() {
    ...
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Version task", "description": "#v2.0 release"}),
    )
    .await;
    dispatch(
        &ctx,
        json!({"op": "add task", "title": "Bugfix task", "description": "#bug-fix found"}),
    )
    .await;
...
async fn s18_dispatch_and_direct_api_return_same_results() {
    ...
    assert_eq!(
        dispatch_titles, direct_titles,
        "dispatch and direct API should return identical results"
    );

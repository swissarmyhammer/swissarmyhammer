...
async fn test_http_mcp_server_rmcp_client_final() {
    ...
    let mut server = start_mcp_server(mode, None)
        .await
    ...
    let test_result = timeout(Duration::from_secs(10), async {
        ...
        let config = SseClientConfig {
            ...
            retry_policy: std::sync::Arc::new(
                rmcp::transport::common::client_side_sse::FixedInterval::default(),
            ),
    ...
    match test_result {
        Ok((tool_count, prompt_count)) => {
            ...
            println!(
                "HTTP MCP server with rmcp client SUCCESS: {} tools, {} prompts",
                tool_count, prompt_count
            );
        }
        Err(_) => {

...
impl DevUtilities {
            ...
            if let Some(ref reason) = metadata.exclusion_reason {
                if reason.len() < 10 || reason.split_whitespace().count() < 3 {
                    return ToolSuggestion::ImproveReason {
                        ...
                        suggestion:
                            "Provide a more descriptive reason explaining why this tool is MCP-only"
                                .to_string(),
                    };
                }
            } else {
                return ToolSuggestion::ImproveReason {
                    ...
                    suggestion:
                        "Add exclusion_reason explaining why this tool is excluded from CLI"
                            .to_string(),
                };
            }
        }
    ...
    pub fn generate_statistics<D: CliExclusionDetector>(detector: &D) -> HashMap<String, usize> {
        ...
        stats.insert(
            "excluded_tools".to_string(),
            analyses.iter().filter(|a| a.is_excluded).count(),
        );
        stats.insert(
            "should_exclude".to_string(),
            analyses
                .iter()
        ...
        stats.insert(
            "correctly_configured".to_string(),
            analyses
                .iter()
                .filter(|a| matches!(a.suggestion, ToolSuggestion::CorrectlyConfigured))
                .count(),
        );
...
impl DocumentationGenerator {
    ...
    pub fn generate_excluded_tools_doc<D: CliExclusionDetector>(detector: &D) -> String {
        let all_metadata = detector.get_all_tool_metadata();
        let excluded_tools: Vec<_> = all_metadata.iter().filter(|m| m.is_cli_excluded).collect();

        let mut doc = String::new();
        doc.push_str("# CLI Excluded Tools\n\n");
        doc.push_str(
            "This document lists all MCP tools that are excluded from CLI generation.\n\n",
        );
    ...
    pub fn generate_dev_report<D: CliExclusionDetector>(detector: &D) -> String {
        ...
        doc.push_str(&format!(
            "- **Need better reasons:** {need_better_reasons}\n"
        ));
        doc.push_str(&format!(
            "- **Correctly configured:** {correctly_configured}\n"
        ));
        doc.push('\n');

        // Suggestions sections
        let should_exclude: Vec<_> = analyses
            .iter()
            .filter(|a| matches!(a.suggestion, ToolSuggestion::ShouldExclude { .. }))
            .collect();

        let should_include: Vec<_> = analyses
            .iter()
            .filter(|a| matches!(a.suggestion, ToolSuggestion::ShouldInclude { .. }))
            .collect();

        let improve_reason: Vec<_> = analyses
            .iter()
            .filter(|a| matches!(a.suggestion, ToolSuggestion::ImproveReason { .. }))
            .collect();
            ...
            for analysis in should_include {
                if let ToolSuggestion::ShouldInclude { reason } = &analysis.suggestion {
                    ...
                    if let Some(current_reason) = &analysis.exclusion_reason {
                        doc.push_str(&format!(
                            "**Current exclusion reason:** {current_reason}\n\n"
                        ));
                    }
                }
            }
        ...
        if !improve_reason.is_empty() {
            ...
            for analysis in improve_reason {
                if let ToolSuggestion::ImproveReason {
                    current_reason,
                    suggestion,
                } = &analysis.suggestion
                {
...
impl ExclusionValidator {
    ...
    pub fn validate_all<D: CliExclusionDetector>(&self, detector: &D) -> ValidationReport {
        ...
        report.summary.excluded_tools = all_metadata.iter().filter(|m| m.is_cli_excluded).count();
        report.summary.eligible_tools = all_metadata.iter().filter(|m| !m.is_cli_excluded).count();
    ...
    fn validate_exclusion_consistency(
        &self,
        metadata: &[ToolCliMetadata],
        ...
        for tool_meta in metadata {
            if !tool_meta.is_cli_excluded {
                if let Some((reason, confidence)) =
                    self.should_probably_be_excluded(&tool_meta.name)
                {
                    if confidence >= 0.8 {
    ...
    fn validate_exclusion_reasoning(
        &self,
        metadata: &[ToolCliMetadata],
        report: &mut ValidationReport,
    ) {
            ...
            if tool_name.contains(pattern) {
                ...
                if let Some((_, current_confidence, _)) = &best_match {
                    if confidence > *current_confidence {
                        best_match = Some((
                            format!(
                                "Contains workflow indicator '{pattern}' suggesting MCP-only usage"
                            ),
                            confidence,
                            pattern,
                        ));
...
mod tests {
    ...
    fn test_should_probably_be_excluded() {
        ...
        let (reason, confidence) = validator
            .should_probably_be_excluded("workflow_orchestrator")
            .unwrap();
        ...
        // Test no match
        assert!(validator
            .should_probably_be_excluded("normal_tool")
            .is_none());
    }
    ...
    fn test_validate_exclusion_consistency() {
        ...
        metadata_map.insert(
            "issue_work".to_string(),
            ToolCliMetadata::included("issue_work"),
        );
        metadata_map.insert(
            "normal_tool".to_string(),
            ToolCliMetadata::included("normal_tool"),
        );
        metadata_map.insert(
        ...
        assert!(report.has_issues());
        let suggest_issues: Vec<_> = report
            .issues
            .iter()
        ...
        if let ValidationIssue::SuggestExclusion {
            tool_name,
            confidence,
            ..
        } = &suggest_issues[0]
    ...
    fn test_validate_exclusion_reasoning() {
        ...
        metadata_map.insert(
            "excluded_vague_reason".to_string(),
            ToolCliMetadata {
        ...
        metadata_map.insert(
            "excluded_good_reason".to_string(),
        ...
        let missing_reason_issues: Vec<_> = report
            .issues
            .iter()
        ...
        let vague_reason_warnings: Vec<_> = report
            .warnings
            .iter()
    ...
    fn test_validate_all_summary() {
        ...
        metadata_map.insert(
            "excluded1".to_string(),
            ToolCliMetadata::excluded("excluded1", "Good reason"),
        );
    ...
    fn test_dev_utilities_correctly_configured() {
        ...
        let excluded_metadata =
            ToolCliMetadata::excluded("issue_work", "MCP workflow management only");
    ...
    fn test_dev_utilities_statistics() {
        ...
        metadata_map.insert(
            "tool_work".to_string(),
            ToolCliMetadata::included("tool_work"),
        );
        metadata_map.insert(
            "properly_excluded".to_string(),
            ToolCliMetadata::excluded("properly_excluded", "Good reason for exclusion"),
        );
        metadata_map.insert(
            "poorly_excluded_work".to_string(),
            ToolCliMetadata::excluded("poorly_excluded_work", "Bad"),
        );
        metadata_map.insert(
            "normal_tool".to_string(),
            ToolCliMetadata::included("normal_tool"),
    ...
    fn test_dev_utilities_find_potential_exclusions() {
        ...
        metadata_map.insert(
            ...
            ToolCliMetadata::included("workflow_orchestrator"),
        );
        metadata_map.insert(
            "normal_tool".to_string(),
            ToolCliMetadata::included("normal_tool"),
    ...
    fn test_dev_utilities_find_potential_inclusions() {
        let mut metadata_map = HashMap::new();
        metadata_map.insert(
            "excluded_no_pattern".to_string(),
            ToolCliMetadata::excluded("excluded_no_pattern", "Some reason"),
        );
        metadata_map.insert(
            "properly_excluded".to_string(),
            ToolCliMetadata::excluded("issue_work", "Good reason"),
        );
    ...
    fn test_documentation_generator_excluded_tools_doc() {
        ...
        metadata_map.insert(
            "workflow_orchestrator".to_string(),
            ToolCliMetadata::excluded("workflow_orchestrator", "Internal workflow coordination"),
        );
        metadata_map.insert(
            "normal_tool".to_string(),
            ToolCliMetadata::included("normal_tool"),
    ...
    fn test_documentation_generator_dev_report() {
        ...
        metadata_map.insert(
            "tool_work".to_string(),
            ToolCliMetadata::excluded("tool_work", "Bad"),
        ); // Improve reason
        metadata_map.insert(
            "normal_tool".to_string(),
            ToolCliMetadata::included("normal_tool"),
    ...
    fn test_documentation_generator_categorize_reason() {
        assert_eq!(
            DocumentationGenerator::categorize_reason("MCP workflow management"),
            "MCP Workflow Tools"
        );
        assert_eq!(
            DocumentationGenerator::categorize_reason("workflow orchestration only"),
            "MCP Workflow Tools"
        );
        assert_eq!(
            DocumentationGenerator::categorize_reason("internal use only"),
            "Internal Tools"
        );
        assert_eq!(
            DocumentationGenerator::categorize_reason("experimental feature"),
            "Experimental/Testing Tools"
        );
        assert_eq!(
            DocumentationGenerator::categorize_reason("deprecated API"),
            "Deprecated Tools"
        );
        assert_eq!(
            DocumentationGenerator::categorize_reason("some other reason"),
            "Other Exclusions"
        );
    }
    ...
    fn test_comprehensive_cli_exclusion_system_integration() {
        ...
        metadata_map.insert(
            ...
            ToolCliMetadata::excluded(
                "issue_merge",
                "MCP workflow state management requiring coordinated transitions",
            ),
        );
        metadata_map.insert(
            "abort_create".to_string(),
            ToolCliMetadata::excluded(
                "abort_create",
                "MCP-specific error handling mechanism for workflow termination",
            ),
        );
        ...
        metadata_map.insert(
            "normal_excluded".to_string(),
            ToolCliMetadata::excluded("normal_excluded", "Some reason"),
        );

        // Correctly configured tools
        metadata_map.insert(
            "files_read".to_string(),
            ToolCliMetadata::included("files_read"),
        );
        metadata_map.insert(
            "web_search".to_string(),
            ToolCliMetadata::included("web_search"),
        );
        ...
        for analysis in &analyses {
            match analysis.name.as_str() {
                "issue_work" => {
                    assert!(
                        matches!(analysis.suggestion, ToolSuggestion::ShouldExclude { confidence, .. } if confidence == 1.0)
                    );
                    assert!(analysis
                        .pattern_matches
                        .iter()
                        .any(|(pattern, _)| pattern == "issue_work"));
                }
                "workflow_orchestrator" => {
                    assert!(
                        matches!(analysis.suggestion, ToolSuggestion::ShouldExclude { confidence, .. } if confidence >= 0.8)
                    );
                    assert!(analysis
                        .pattern_matches
                        .iter()
                        .any(|(pattern, _)| pattern == "workflow_"));
                }
                "normal_excluded" => {
                    assert!(matches!(
                        analysis.suggestion,

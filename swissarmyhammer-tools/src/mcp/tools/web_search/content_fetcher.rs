...
impl ContentFetcher {
    ...
    /// Fetch content from multiple search results concurrently with privacy features
    pub async fn fetch_search_results_with_privacy(
        &self,
        ...
        debug!(
            "Privacy-aware content fetching completed: {} successful, {} failed, {} rate limited, {} quality filtered in {}ms",
    ...
    /// Fetch content from a single search result with privacy features
    async fn fetch_single_result_with_privacy(
        &self,
        ...
        // Apply privacy jitter delay for content requests
        privacy_manager.apply_jitter().await;

        let start_time = Instant::now();

        // Build request with privacy features
        let mut request_builder = self.client.get(&result.url);
        ...
        debug!(
            "Content fetched from {}: {} words in {}ms",
            result.url, word_count, fetch_time_ms
        );

        // Record domain use for rate limiting

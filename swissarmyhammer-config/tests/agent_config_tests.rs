...
use swissarmyhammer_config::agent::{AgentManager, AgentSource, AgentError, parse_agent_description, parse_agent_config};
use std::{env, fs};
...
fn test_agent_manager_load_builtin_agents_comprehensive() {
    ...
    assert!(agent_names.contains(&"claude-code"), "Should have claude-code");
    assert!(agent_names.contains(&"qwen-coder"), "Should have qwen-coder");
    assert!(agent_names.contains(&"qwen-coder-flash"), "Should have qwen-coder-flash");
    ...
    for agent in &agents {
        ...
        assert!(parsed_config.is_ok(), "Builtin agent '{}' should have valid config", agent.name);
    }
}
...
executor:
  ...
  config:
    ...
    args: ["--user-override"]
quiet: true"#;
    fs::write(user_agents_dir.join("claude-code.yaml"), user_claude_override)
...
executor:
  ...
  config:
    ...
    mcp_server:
      ...
      timeout_seconds: 30
quiet: false"#;
    fs::write(project_agents_dir.join("qwen-coder.yaml"), project_qwen_override)
...
executor:
  ...
  config:
    ...
    args: ["--project-mode"]
quiet: true"#;
    fs::write(project_agents_dir.join("project-specific.yaml"), project_specific)
    ...
    let agent_map: std::collections::HashMap<_, _> = agents.iter()
        .map(|agent| (agent.name.as_str(), agent))
        .collect();
    
    // claude-code should come from user (highest precedence for user override)
    let claude_agent = agent_map.get("claude-code").expect("Should have claude-code");
    assert_eq!(claude_agent.source, AgentSource::User, "claude-code should be from user source");
    assert!(claude_agent.description.as_ref().unwrap().contains("User override"), 
    ...
    assert_eq!(qwen_agent.source, AgentSource::Project, "qwen-coder should be from project source");
    assert!(qwen_agent.description.as_ref().unwrap().contains("Project override"),
            "Should have project override description");
    
    // qwen-coder-flash should remain builtin (no overrides)
    let qwen_flash_agent = agent_map.get("qwen-coder-flash").expect("Should have qwen-coder-flash");
    assert_eq!(qwen_flash_agent.source, AgentSource::Builtin, "qwen-coder-flash should remain builtin");
    
    // Should have user-custom agent
    let user_custom_agent = agent_map.get("user-custom").expect("Should have user-custom");
    assert_eq!(user_custom_agent.source, AgentSource::User);
    
    // Should have project-specific agent  
    let project_specific_agent = agent_map.get("project-specific").expect("Should have project-specific");
...
fn test_agent_manager_config_file_operations() {
    ...
    assert!(config_path.parent().unwrap().exists(), "Config directory should exist");
    assert_eq!(config_path.file_name().unwrap(), "sah.yaml", "Should default to YAML format");
    ...
    assert!(config_content.contains("agent:"), "Should contain agent section");
    assert!(config_content.contains("executor:"), "Should contain executor config");
    assert!(config_content.contains("claude-code"), "Should contain claude-code config");
    ...
    let updated_content = fs::read_to_string(&config_path)
        .expect("Should read updated config");
    assert!(updated_content.contains("agent:"), "Should still contain agent section");
    assert!(updated_content.contains("llama-agent") || updated_content.contains("qwen-coder"), 
            "Should contain new agent config");
    
    let updated_size = fs::metadata(&config_path).unwrap().len();
    assert_ne!(original_size, updated_size, "Config file should be modified");
...
"#;
    ...
    assert!(updated_config.contains("default_template"), "Should preserve prompt config");
    assert!(updated_config.contains("workflows:"), "Should preserve workflows section");
    assert!(updated_config.contains("test-workflow"), "Should preserve workflow data");
    assert!(updated_config.contains("other_section:"), "Should preserve other_section");
    assert!(updated_config.contains("preserved_value"), "Should preserve nested data");
    assert!(updated_config.contains("nested:"), "Should preserve nested structure");
...
fn test_agent_manager_error_handling_comprehensive() {
    ...
    assert!(find_result.is_err(), "Should fail to find nonexistent agent");
...
invalid_field: true"#;
    ...
    fs::write(user_agents_dir.join("not-agent.txt"), "This is not an agent file")
    ...
    let user_agents = AgentManager::load_user_agents()
        .expect("Should load user agents despite invalid files");
    ...
    assert!(user_agents[0].description.as_ref().unwrap().contains("Valid test agent"));
    
    // Test that invalid agents are not in the list
    let agent_names: Vec<_> = user_agents.iter().map(|a| a.name.as_str()).collect();
    assert!(!agent_names.contains(&"invalid-syntax"), "Should not load invalid syntax");
    assert!(!agent_names.contains(&"invalid-config"), "Should not load invalid config");
    assert!(!agent_names.contains(&"not-agent"), "Should not load non-yaml file");
}
...
quiet: false"#;
    ...
    assert_eq!(description, Some("YAML frontmatter description".to_string()));
...
quiet: false"#;
    ...
    let config = parse_agent_config(pure_config_content)
        .expect("Should parse pure agent config");
...
fn test_agent_manager_directory_loading_edge_cases() {
    ...
    assert_eq!(result.unwrap().len(), 0, "Should return empty list for non-existent dir");
    ...
    assert_eq!(result.unwrap().len(), 0, "Should return empty list for empty dir");
    ...
    fs::write(non_yaml_dir.join("readme.txt"), "Not an agent")

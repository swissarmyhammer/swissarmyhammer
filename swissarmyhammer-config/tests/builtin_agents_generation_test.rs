...
fn test_builtin_agents_generation() {
    ...
    for (name, content) in agents {
        ...
        assert!(content.contains("agent:"), "Agent content should contain 'agent:' key for {}", name);
        assert!(content.contains("executor:"), "Agent content should contain 'executor:' key for {}", name);
    }
}
...
fn test_builtin_agents_specific_content() {
    ...
    let claude_content = agents_map.get("claude-code").expect("claude-code agent should exist");
    assert!(claude_content.contains("type: claude-code"));
    
    // Test qwen-coder agent
    let qwen_content = agents_map.get("qwen-coder").expect("qwen-coder agent should exist");
    ...
    let qwen_flash_content = agents_map.get("qwen-coder-flash").expect("qwen-coder-flash agent should exist");

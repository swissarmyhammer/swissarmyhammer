...
fn test_complex_env_substitution_patterns() {
    ...
    ctx.set(
        ...
        serde_json::Value::String("${MISSING_VAR:-default_value}".to_string()),
    );
    ctx.set(
        "multiple_vars".to_string(),
        serde_json::Value::String("${PROTOCOL}://${HOST}:${PORT}".to_string()),
    );
    ctx.set(
        "mixed_content".to_string(),
        serde_json::Value::String("API endpoint: ${PROTOCOL}://${HOST}:${PORT}/api/v1".to_string()),
    );
    ctx.set(
    ...
    assert_eq!(
        ctx.get("with_default"),
        Some(&serde_json::Value::String("default_value".to_string()))
    );
    assert_eq!(
        ctx.get("multiple_vars"),
        Some(&serde_json::Value::String(
            "https://api.example.com:8080".to_string()
        ))
    );
    assert_eq!(
        ctx.get("mixed_content"),
        Some(&serde_json::Value::String(
            "API endpoint: https://api.example.com:8080/api/v1".to_string()
        ))
    );
    ...
    if let Some(nested) = ctx.get("nested_structure") {
        assert_eq!(
            nested["connection"]["url"],
            serde_json::Value::String("https://api.example.com:8080".to_string())
        );
        assert_eq!(
            nested["connection"]["timeout"],
            serde_json::Value::String("30".to_string())
        );
        assert_eq!(
            nested["connection"]["credentials"]["api_key"],
            serde_json::Value::String("secret_key_123".to_string())
        );
        assert_eq!(
            nested["connection"]["credentials"]["token"],
            serde_json::Value::String("temp_token".to_string())
        );
    } else {
...
fn test_env_substitution_edge_cases() {
    ...
    ctx.set(
        "space_in_default".to_string(),
        serde_json::Value::String("${MISSING:-default with spaces}".to_string()),
    );
    ctx.set(
        "adjacent_vars".to_string(),
        serde_json::Value::String("${EDGE_TEST}${EDGE_TEST}".to_string()),
    );
    ctx.set(
        "var_with_text".to_string(),
        serde_json::Value::String("prefix_${EDGE_TEST}_suffix".to_string()),
    );
    ctx.set(
        "special_chars_in_default".to_string(),
        serde_json::Value::String("${MISSING:-default:with:colons}".to_string()),
    );

    ctx.substitute_env_vars().unwrap();

    assert_eq!(
        ctx.get("empty_default"),
        Some(&serde_json::Value::String("".to_string()))
    );
    assert_eq!(
        ctx.get("space_in_default"),
        Some(&serde_json::Value::String(
            "default with spaces".to_string()
        ))
    );
    assert_eq!(
        ctx.get("adjacent_vars"),
        Some(&serde_json::Value::String(
            "edge_valueedge_value".to_string()
        ))
    );
    assert_eq!(
        ctx.get("var_with_text"),
        Some(&serde_json::Value::String(
            "prefix_edge_value_suffix".to_string()
        ))
    );
    assert_eq!(
        ctx.get("special_chars_in_default"),
        Some(&serde_json::Value::String(
            "default:with:colons".to_string()
        ))
    );
...
fn test_env_substitution_in_arrays() {
    ...
    if let Some(serde_json::Value::Array(arr)) = ctx.get("config_array") {
        ...
        assert_eq!(
            arr[3],
            serde_json::Value::String("default_item".to_string())
        );
...
fn test_env_substitution_preserves_non_strings() {
    ...
    ctx.set(
        "string_with_sub".to_string(),
        serde_json::Value::String("value: ${STRING_VAR}".to_string()),
    );
    ctx.set(
        "object".to_string(),
        serde_json::json!({
    ...
    assert_eq!(
        ctx.get("string_with_sub"),
        Some(&serde_json::Value::String("value: substituted".to_string()))
    );

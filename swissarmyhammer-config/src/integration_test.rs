...
"#).unwrap();
    ...
    fn test_template_context_operations() {
        ...
        other_ctx.set("other_key".to_string(), serde_json::Value::String("other_value".to_string()));
        
        ctx.merge(&other_ctx);
        assert_eq!(ctx.get("test_key"), Some(&serde_json::Value::String("test_value".to_string())));
        assert_eq!(ctx.get("other_key"), Some(&serde_json::Value::String("other_value".to_string())));
    }
    ...
    fn test_env_var_substitution_basic() {
        ...
        let mut ctx = TemplateContext::new();
        ctx.set("config_var".to_string(), serde_json::Value::String("${TEST_ENV_VAR}".to_string()));
        
        ctx.substitute_env_vars().unwrap();
        
        assert_eq!(ctx.get("config_var"), Some(&serde_json::Value::String("test_env_value".to_string())));

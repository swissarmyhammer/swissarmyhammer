...
pub async fn start_mcp_server(
    ...
    start_mcp_server_with_options(mode, library, working_dir).await
}
...
pub async fn start_mcp_server_with_options(
    ...
    match mode {
        McpServerMode::Stdio => start_stdio_server(library, working_dir).await,
        McpServerMode::Http { port } => start_http_server(port, library, working_dir).await,
    }
}
...
async fn start_stdio_server(
    ...
    let work_dir = working_dir
        .clone()
        .unwrap_or_else(|| std::env::current_dir().unwrap_or_else(|_| std::env::temp_dir()));
    let temp_server = McpServer::new_with_work_dir(library, work_dir).await?;
    ...
    let server_arc = initialize_mcp_server(None, http_port, working_dir).await?;
...
async fn start_http_server(
    ...
    let server_arc = initialize_mcp_server(library, actual_port, working_dir).await?;
...
mod tests {
    ...
    async fn test_start_mcp_server_with_options_binds_port() {
        ...
        let mut server = start_mcp_server_with_options(mode, None, None)
            .await
            .unwrap();
    ...
    async fn test_start_mcp_server_with_options_working_dir() {
        ...
        let mut server =
            start_mcp_server_with_options(mode, None, Some(temp_dir.path().to_path_buf()))
                .await
                .unwrap();
        assert!(server.port().unwrap() > 0);
        server.shutdown().await.unwrap();
    ...
    async fn test_start_mcp_server_cli_mode_skips_logging() {
        ...
        let result = start_mcp_server_with_options(mode, None, None).await;

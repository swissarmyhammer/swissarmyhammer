...
pub(crate) fn perspective_scope(p: &Perspective) -> &str {
...
///
/// Mirrors the `is_safe_entity_type` storage-path guard in
/// `crate::context`: rejects empty strings, leading `.`, path separators
...
/// scope instead.
pub(crate) fn is_safe_scope_component(scope: &str) -> bool {
    !scope.is_empty()
...
pub(crate) fn matches_scope(p: &Perspective, view_id: Option<&str>, view_kind: &str) -> bool {
...
/// reconciliation.
fn is_customized(p: &Perspective) -> bool {
...
mod tests {
    ...
    #[tokio::test]
    async fn matches_scope_follows_view_id_first_kind_fallback() {
        let pinned = vanilla("01AAA", "board", Some("view-1"));
        assert!(matches_scope(&pinned, Some("view-1"), "board"));
        assert!(!matches_scope(&pinned, Some("view-2"), "board"));
        // No incoming view_id: kind fallback applies even to pinned ones.
        assert!(matches_scope(&pinned, None, "board"));

        let legacy = vanilla("01BBB", "board", None);

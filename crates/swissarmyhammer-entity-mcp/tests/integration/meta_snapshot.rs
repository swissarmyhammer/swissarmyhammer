//! Snapshot test for the `_meta` operations tree of the `entity` tool.
//!
...
/// (noun, verb, op) tuple for the `entity` tool. This snapshot pins the
/// current set; a deliberate addition / rename should update this assertion
/// in the same PR as the operation struct change.
#[tokio::test]
async fn entity_tool_meta_operations_tree_is_complete() {
    ...
    assert_eq!(tool.name.as_ref(), "entity");

    let meta = tool
        .meta
        .as_ref()
        .expect("entity tool advertises a _meta tree");
    let ops_tree = meta
        .0
    ...
    // Catches drift between the wire schema and the _meta tree (both
    // generated from the same operation slice).

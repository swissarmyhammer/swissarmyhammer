...
pub enum ToolCallContent {
    ...
    Content {
        ...
        ///
        /// Boxed because `ContentBlock` is by far the largest payload of any
        /// `ToolCallContent` variant (~360 bytes vs ~72 for `Diff`); boxing
    ...
    pub fn to_acp_content(&self) -> agent_client_protocol::schema::ToolCallContent {
        match self {
            ToolCallContent::Content { content } => {
                ...
                let content_block = content.as_ref().clone();

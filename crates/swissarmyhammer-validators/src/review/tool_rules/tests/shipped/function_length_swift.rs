...
/// The `function-length-swift` probe of a run a project configuration declines.
///
/// The judged file holds the long function under the directory the project
/// excludes, so a run that still reads that file reports the ONE row the
/// function head opens.
fn swift_length_project_decline_probe() -> SwiftProjectDeclineProbe {
    SwiftProjectDeclineProbe {
        rule: SWIFT_FUNCTION_LENGTH_RULE,
        source: swift_staged_source(),
        heads: vec![format!("func {SWIFT_STAGED_FUNCTION_NAME}(")],
    }
}
...
fn the_shipped_swift_function_length_tool_rule_declines_a_run_the_project_excludes_whole() {
    verify_swift_project_decline_is_stated(
        &swift_length_project_decline_probe(),
...
/// second run drops the project's `excluded:` list. The long function under the
/// excluded directory then reports its one row, so this rule measured the code
/// with settings the project did not ask for and has to say so.
#[test]

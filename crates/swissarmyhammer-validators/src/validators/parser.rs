...
/// Legacy frontmatter keys that the loader silently ignores but `check
/// validators` flags, paired with the guidance shown to the user.
///
/// Adding a row here makes a stray key both lenient at load time (serde ignores
/// unknown keys) and strict at check time (this table drives the warning).
const LEGACY_MANIFEST_KEYS: &[(&str, &str)] = &[(
    "trigger",
    "hook triggers were removed; delete the `trigger` key",
)];

/// Lint a RuleSet `VALIDATOR.md` manifest for stray/legacy frontmatter keys.
///
/// This is the strict counterpart to the lenient loader: parsing a manifest
/// ignores unknown keys (so a leftover `trigger:` still loads), while
/// `check validators` calls this to surface those keys so `sah doctor` can nudge
/// the user to clean them up.
///
/// Returns a human-readable issue string for each stray key found. An empty
/// vector means the manifest frontmatter is clean. Malformed YAML or missing
/// frontmatter yields no issues here — those are reported by the parser.
///
/// # Arguments
///
/// * `content` - The full `VALIDATOR.md` file content.
/// * `dir_path` - The RuleSet directory path (for the issue message).
pub fn check_manifest_frontmatter(content: &str, dir_path: &Path) -> Vec<String> {
    let Ok((frontmatter_str, _body)) = extract_frontmatter(content, dir_path) else {
        return Vec::new();
    };

    let rendered = render_frontmatter(frontmatter_str);

    let Ok(serde_yaml_ng::Value::Mapping(mapping)) =
        serde_yaml_ng::from_str::<serde_yaml_ng::Value>(&rendered)
    else {
        return Vec::new();
    };
    ...
    LEGACY_MANIFEST_KEYS
        .iter()
        .filter(|(key, _)| mapping.contains_key(serde_yaml_ng::Value::from(*key)))
        .map(|(key, guidance)| {
            format!(
                "{}/VALIDATOR.md: stray `{}` key — {}",
                dir_path.display(),
                key,
                guidance
            )
...
"#;
        ...
        let mut expander = YamlExpander::<ValidatorsConfig>::new();
        expander
            .add_builtin(
                "file_groups/source_code",
                r#"
...
# Test
"#;
        let manifest = parse_ruleset_manifest::<swissarmyhammer_directory::ValidatorsConfig>(
            content,
            Path::new("test"),
            None,
        )
...
"#,
    ...
    fn test_parse_ruleset_directory_not_a_directory() {
        let result = parse_ruleset_directory::<swissarmyhammer_directory::ValidatorsConfig>(
...
Body.
"#;
        let result = parse_ruleset_manifest::<swissarmyhammer_directory::ValidatorsConfig>(
            content,
...
"#;
    ...
    #[test]
    fn test_check_manifest_flags_stray_trigger() {
        // `check validators` is STRICT: a leftover legacy `trigger:` is flagged so
        // doctor nudges the user to remove it.
        let content = r#"---
name: security-rules
description: Critical security validations
trigger: PostToolUse
...
# Security Rules
"#;
        let issues = check_manifest_frontmatter(content, std::path::Path::new("security-rules"));
        assert!(
            issues.iter().any(|issue| issue.contains("trigger")),
            "stray `trigger` should be flagged, got: {issues:?}"
        );
    }

    #[test]
    fn test_check_manifest_clean_for_modern_frontmatter() {
        // A modern validator (no `trigger`, with `probes`) produces no issues.
        let content = r#"---
name: dead-code
...
# Dead Code
"#;
        let issues = check_manifest_frontmatter(content, std::path::Path::new("dead-code"));
        assert!(
            issues.is_empty(),
            "modern frontmatter is clean, got: {issues:?}"
        );
    }

    #[test]
    fn test_parse_ruleset_manifest_parses_probes() {
        let content = r#"---
name: dead-code
description: Flags symbols with no inbound callers
probes:
...
"#;
        let manifest = parse_ruleset_manifest::<swissarmyhammer_directory::ValidatorsConfig>(
            content,
            std::path::Path::new("dead-code"),
            None,
        )
        .unwrap();
        assert_eq!(manifest.probes, vec!["callers".to_string()]);
    }

    #[test]
    fn test_parse_ruleset_manifest_probes_default_empty() {
        let content = r#"---
name: data-driven
...
"#;
    ...
    #[test]
    fn test_parse_ruleset_manifest_lenient_ignores_stray_trigger() {
        // A leftover legacy `trigger:` key must NOT break loading (loader is lenient).
        let content = r#"---
name: security-rules
description: Critical security validations
trigger: PostToolUse
...
"#;
        ...
        .unwrap();
        assert_eq!(manifest.name, "security-rules");
        assert_eq!(manifest.probes, vec!["duplicates".to_string()]);
    }

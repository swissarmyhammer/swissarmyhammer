//! Adapter for using a template library as Liquid template partials
//!
//! This module provides the integration between template libraries and
//! the Liquid template engine's partial system.
//!
//! Note: This is a thin wrapper around the generic [`LibraryPartialAdapter`].
//! The template library implements [`TemplateContentProvider`](crate::partials::TemplateContentProvider),
//! which enables it to work with the unified partial adapter system.

use crate::partials::LibraryPartialAdapter;
use crate::prompts::TemplateLibrary;
use std::sync::Arc;

/// Adapter that allows a template library to be used as Liquid template partials.
///
/// This is a type alias for the generic [`LibraryPartialAdapter`] specialized
/// for [`TemplateLibrary`]. The underlying library implements
/// [`TemplateContentProvider`](crate::partials::TemplateContentProvider),
/// enabling it to work with the unified partial system.
pub type PartialAdapter = LibraryPartialAdapter<TemplateLibrary>;

/// Create a new partial adapter from a library Arc.
///
/// This is a convenience function that creates a [`PartialAdapter`]
/// (which is a `LibraryPartialAdapter<TemplateLibrary>`).
pub fn new_partial_adapter(library: Arc<TemplateLibrary>) -> PartialAdapter {
...
mod tests {
    ...
    use crate::prompts::{Prompt, TemplateLibrary};
    use crate::PartialLoader;
    ...
    fn test_convenience_function() {
        ...
        let adapter = new_partial_adapter(Arc::new(library));

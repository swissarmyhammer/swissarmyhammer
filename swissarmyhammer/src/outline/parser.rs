...
use crate::outline::extractors::{RustExtractor, TypeScriptExtractor, JavaScriptExtractor, DartExtractor};
...
impl OutlineParser {
    ...
    pub fn parse_file(&self, file_path: &Path, content: &str) -> Result<OutlineTree> {
        ...
        let extractor = self.extractors.get(&language)
            .ok_or_else(|| crate::outline::OutlineError::LanguageDetection(
                format!("No symbol extractor available for language: {:?}", language)
            ))?;
        
        // Parse with Tree-sitter using a custom approach for outline extraction
        let symbols = self.parse_with_treesitter(file_path, content, &language, extractor.as_ref())?;
    ...
    fn parse_with_treesitter(
        ...
        let tree_sitter_language = match language {
            ...
            Language::Unknown => return Err(crate::outline::OutlineError::LanguageDetection(
        ...
        parser.set_language(&tree_sitter_language)
            .map_err(|e| crate::outline::OutlineError::TreeSitter(
                format!("Failed to set parser language for {:?}: {}", language, e)
            ))?;
        
        // Parse the content
        let tree = parser.parse(content, None)
            .ok_or_else(|| crate::outline::OutlineError::TreeSitter(
                format!("Failed to parse {} with Tree-sitter", file_path.display())
            ))?;
...
pub mod inner {
    pub fn inner_function() {}
        ...
        assert!(functions.len() >= 2, "Should find test_function and private_helper");
        ...
        assert!(!test_struct_symbols.is_empty(), "Should find TestStruct by name");
        
        // Test documentation extraction
        let documented_symbols: Vec<_> = outline.all_symbols()
            .into_iter()
        ...
        let signatures: Vec<_> = outline.all_symbols()
            .into_iter()

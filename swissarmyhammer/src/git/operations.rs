...
//! ## Implementation
//!
//! This module uses git2-rs for all git operations, providing:
//! - **High Performance**: Native git operations without subprocess overhead
//! - **Structured Errors**: Detailed error information for better error handling
//! - **Memory Efficiency**: Direct access to git internals
//! - **Cross-platform**: Consistent behavior across all supported platforms
//!
//! ## Core Functionality
//!
//! - **Repository Management**: Repository verification and initialization
//! - **Branch Operations**: Creating, switching, and deleting branches
//! - **Working Directory**: Status checking and change detection
//! - **Commit Operations**: Creating commits and managing history
//! - **Merge Operations**: Merging branches and resolving conflicts
...
pub struct GitOperations {
    /// Working directory for git operations
    work_dir: PathBuf,
    /// Git2 repository handle for native operations
...
impl GitOperations {
    ...
    /// Create GitOperations with automatic detection
    pub fn new_auto() -> Result<Self> {
        let work_dir = std::env::current_dir()?;
        Self::with_work_dir(work_dir)
    }

    /// Verify directory is a git repository using git2-rs
    ...
    ///
    /// This method is no longer needed since git2_repo is always initialized.
    pub fn init_git2(&mut self) -> Result<()> {
        // git2_repo is always initialized now, so this is a no-op
        Ok(())
    }

    /// Get reference to git2 repository
    ///
    /// This method provides access to the git2::Repository handle.
    /// The repository handle is always available since it's initialized in the constructor.
    ...
    pub fn has_git2_repo(&self) -> bool {
        true // Always true since git2_repo is always initialized
    }
    ...
    fn get_git2_repo(&self) -> &Repository {
        &self.git2_repo
    }
    ...
    /// Merge branch using git2
    fn merge_branch(&self, source_branch: &str, target_branch: &str, message: &str) -> Result<()> {
    ...
    pub fn merge_issue_branch_auto(&self, issue_name: &str) -> Result<String> {
        self.merge_issue_branch_auto_git2(issue_name)
    }
    ...
    pub fn merge_issue_branch_auto_git2(&self, issue_name: &str) -> Result<String> {
        ...
        self.checkout_branch(&target_branch).map_err(|e| {
    ...
    fn validate_source_branch_state(&self, source_branch: &str, issue_name: &str) -> Result<()> {
        ...
        let repo = open_repository(&self.work_dir)?;
        let _commit_hash = match rev_parse(&repo, source_branch) {
            Ok(hash) => hash,
            Err(_) => {
                return Err(SwissArmyHammerError::git_branch_operation_failed(
                    "validate",
    ...
    ///
    /// Future enhancements may read from .swissarmyhammer/config to allow users
    /// to specify custom target branches on a per-project or per-issue basis.
    ...
    /// Check if git command is available via shell
    /// Since we've migrated to libgit2, shell git is no longer required
    fn is_shell_git_available(&self) -> bool {
        false // Shell git is no longer used after migration to libgit2
    }

    /// Repository verification using git2
    pub fn verify_repository(&self) -> Result<()> {
        Self::verify_git_repo_git2(&self.work_dir)
    }
    ...
    fn test_operation_git2(&self, operation: &str) -> Result<String> {
        match operation {
            ...
            "branch_exists_main" => Ok(self.branch_exists("main")?.to_string()),
            "working_directory_status" => {
                let summary = self.get_status_summary()?;
                Ok(format!(
                    "clean: {}, changes: {}",
...
mod tests {
    ...
    /// Test helper that runs the test with git2 backend
    fn test_with_git2_backend<F>(test_fn: F) -> Result<()>
    where
        F: Fn(&GitOperations) -> Result<()>,
    {
        // Test with git2 backend using a fresh repository
        let git2_temp_dir = create_test_git_repo()?;
        let git2_repo_path = git2_temp_dir.path().to_path_buf();
        let git2_ops = GitOperations::with_work_dir(git2_repo_path)?;
        test_fn(&git2_ops)
    ...
    fn create_test_git_ops() -> Result<GitOperations> {
        let temp_dir = create_test_git_repo()?;
        GitOperations::with_work_dir(temp_dir.path().to_path_buf())
    }

    /// Test git2 backend operation and return result
    fn test_git2_operation<T, F>(operation: F) -> Result<T>
    where
        F: Fn(&GitOperations) -> Result<T>,
    {
        ...
        let git2_ops = GitOperations::with_work_dir(repo_path)?;
        operation(&git2_ops)
    }
    ...
    fn test_backend_infrastructure() {
        ...
        let _shell_ops = create_test_git_ops().unwrap();
        let _git2_ops = create_test_git_ops().unwrap();
    ...
    fn test_current_branch_both_backends() {
        ...
        match test_with_git2_backend(|git_ops| {
    ...
    fn test_git2_backend_works() {
        ...
        // Test git2 backend
        let git2_ops = GitOperations::with_work_dir(repo_path.clone()).unwrap();
    ...
    fn test_git2_performance() {
        // This test verifies git2 backend performance
        let temp_dir = create_test_git_repo().unwrap();
        let repo_path = temp_dir.path().to_path_buf();

        // Create git2 backend instance
        let git2_ops = GitOperations::with_work_dir(repo_path).unwrap();
        ...
        println!("Git2 Backend Performance ({} iterations):", ITERATIONS);
        ...
        // Operations should be reasonably fast (< 100ms per operation for simple repos)
        assert!(git2_current_branch_time.as_millis() < 100);
        assert!(git2_main_branch_time.as_millis() < 100);
        assert!(git2_branch_exists_time.as_millis() < 100);
    }
    ...
    fn test_backend_result_compatibility() {
        ...
        // Test current_branch with git2
        match test_git2_operation(|ops| ops.current_branch()) {
            Ok(git2_branch) => {
                assert!(git2_branch == "main" || git2_branch == "master");
                println!("✓ Git2 backend test passed for current_branch");
            }
            Err(e) => {
        ...
        // Test main_branch with git2
        match test_git2_operation(|ops| ops.main_branch()) {
            Ok(git2_main) => {
                assert!(git2_main == "main" || git2_main == "master");
                println!("✓ Git2 backend test passed for main_branch");
            }
            Err(e) => {
                // Expected: Shell backend fails, verify git2 backend works
                match create_test_git_repo() {
        ...
        // Test branch_exists with git2
        match test_git2_operation(|ops| {
            let main_branch = ops.main_branch()?;
            ops.branch_exists(&main_branch)
        }) {
            Ok(git2_exists) => {
                assert!(git2_exists); // Main branch should exist
                println!("✓ Git2 backend test passed for branch_exists");
            }
            Err(e) => {
                // Expected: Shell backend fails, verify git2 backend works
                match create_test_git_repo() {
    ...
    fn test_current_branch() {
        ...
        test_with_git2_backend(|git_ops| {
            let current_branch = git_ops.current_branch()?;
    ...
    fn test_create_work_branch_from_feature_branch_succeeds() {
        ...
        let main_branch = git_ops.main_branch().unwrap();
        git_ops.checkout_branch(&main_branch).unwrap();
        // Create and switch to new branch using libgit2
        git_ops.create_branch("feature/new-feature").unwrap();
        git_ops.checkout_branch("feature/new-feature").unwrap();
    ...
    fn test_complete_feature_branch_workflow() {
        ...
        super::git2_utils::checkout_new_branch(&repo, "feature/user-auth", None).unwrap();

        // Add initial feature work
        fs::write(temp_dir.path().join("auth.rs"), "// Auth module").unwrap();
        super::git2_utils::add_files(&repo, &["auth.rs"]).unwrap();
        super::git2_utils::create_commit(&repo, "Initial auth module", None, None).unwrap();
        ...
        super::git2_utils::add_files(&repo, &["auth_tests.rs"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add auth tests", None, None).unwrap();
    ...
    fn test_multiple_issues_from_same_source_branch() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "release/v1.0", None).unwrap();
    ...
    fn test_merge_issue_to_correct_source_branch() {
        ...
        let main_branch = git_ops.main_branch().unwrap();
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "develop", None).unwrap();

        // Add file to develop branch to differentiate it
        fs::write(temp_dir.path().join("develop.txt"), "develop branch file").unwrap();
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::add_files(&repo, &["develop.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add develop file", None, None).unwrap();
        ...
        super::git2_utils::add_files(&repo, &["feature.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add feature", None, None).unwrap();
    ...
    fn test_create_work_branch_with_explicit_source() {
        ...
        let _main_branch = git_ops.main_branch().unwrap();
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature/api", None).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature/ui", None).unwrap();
    ...
    fn test_backwards_compatibility_with_simple_methods() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::add_files(&repo, &["test.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Test change", None, None).unwrap();
    ...
    fn test_create_work_branch_explicit_source_compatibility() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature/awesome", None).unwrap();
    ...
    fn test_create_work_branch_uses_current_branch() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "development", None).unwrap();
    ...
    fn test_enhanced_merge_conflict_handling() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature/conflict", None).unwrap();
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::add_files(&repo, &["conflict.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add conflict file on main", None, None).unwrap();
        ...
        super::git2_utils::add_files(&repo, &["issue.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add issue content", None, None).unwrap();
    ...
    fn test_source_branch_state_validation() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "valid_branch", None).unwrap();
    ...
    fn test_backward_compatibility_methods() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::add_files(&repo, &["test.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add test file", None, None).unwrap();
    ...
    fn test_create_work_branch_backwards_compatibility() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature/compat", None).unwrap();
    ...
    fn test_delete_branch_existing_succeeds() {
        ...
        // Use direct approach like other working tests instead of test_with_git2_backend
        let temp_dir = create_test_git_repo().unwrap();
        let git_ops = GitOperations::with_work_dir(temp_dir.path().to_path_buf()).unwrap();
    ...
    fn test_delete_branch_nonexistent_then_existing() {
        ...
        // Create the branch
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "test-branch", None).unwrap();
    ...
    fn test_find_merge_target_branch_using_reflog_git2() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature/test-feature", None).unwrap();
    ...
    fn test_find_merge_target_branch_no_reflog_entry() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::create_branch(&repo, "issue/manual-branch", None).unwrap();
    ...
    fn test_find_branch_creation_point() {
        ...
        // Create feature branch and issue branch from it
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature/source", None).unwrap();
    ...
    fn test_merge_branches_git2_fast_forward() {
        ...
        checkout_new_branch(&repo, "feature", None).unwrap();
    ...
    fn test_merge_branches_git2_normal_merge() {
        ...
        // Verify merge commit was created using libgit2
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        let head = repo.head().unwrap();
    ...
    fn test_merge_branches_git2_conflict_detection() {
        ...
        // Create initial commit with a file using libgit2
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        fs::write(temp_dir.path().join("conflict.txt"), "original content\n").unwrap();
        super::git2_utils::add_files(&repo, &["conflict.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Initial commit", None, None).unwrap();

        // Create feature branch and modify the file
        super::git2_utils::checkout_new_branch(&repo, "feature", None).unwrap();
        fs::write(temp_dir.path().join("conflict.txt"), "feature content\n").unwrap();
        super::git2_utils::add_files(&repo, &["conflict.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Feature change", None, None).unwrap();

        // Switch back to main and modify the same file differently
        super::git2_utils::checkout_branch(&repo, "main").unwrap();
        fs::write(temp_dir.path().join("conflict.txt"), "main content\n").unwrap();
        super::git2_utils::add_files(&repo, &["conflict.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Main change", None, None).unwrap();
    ...
    fn test_merge_branches_git2_up_to_date() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "identical", None).unwrap();
        super::git2_utils::checkout_branch(&repo, "main").unwrap();
    ...
    fn test_create_commit_with_parents() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::add_files(&repo, &["file1.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "First parent", None, None).unwrap();

        // Create second parent on a branch
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "branch", None).unwrap();
        fs::write(temp_dir.path().join("file2.txt"), "content2").unwrap();
        super::git2_utils::add_files(&repo, &["file2.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Second parent", None, None).unwrap();
        ...
        // Verify the commit has two parents using libgit2
        let head = repo.head().unwrap();
        let commit = head.peel_to_commit().unwrap();
        let parent_count = commit.parent_count();
    ...
    fn test_handle_merge_conflicts_detailed_reporting() {
        ...
        // Create initial commit with conflicting files using libgit2
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        fs::write(temp_dir.path().join("file1.txt"), "original\n").unwrap();
        fs::write(temp_dir.path().join("file2.txt"), "original\n").unwrap();
        super::git2_utils::add_files(&repo, &["file1.txt", "file2.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Initial", None, None).unwrap();

        // Create conflicting changes on both branches
        super::git2_utils::checkout_new_branch(&repo, "branch1", None).unwrap();
        fs::write(temp_dir.path().join("file1.txt"), "branch1 change\n").unwrap();
        fs::write(temp_dir.path().join("file2.txt"), "branch1 change\n").unwrap();
        super::git2_utils::add_files(&repo, &["file1.txt", "file2.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Branch1 changes", None, None).unwrap();

        super::git2_utils::checkout_branch(&repo, "main").unwrap();
        fs::write(temp_dir.path().join("file1.txt"), "main change\n").unwrap();
        fs::write(temp_dir.path().join("file2.txt"), "main change\n").unwrap();
        super::git2_utils::add_files(&repo, &["file1.txt", "file2.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Main changes", None, None).unwrap();
    ...
    fn create_test_repo_with_commits() -> Result<(TempDir, GitOperations)> {
        ...
        for i in 1..=3 {
            ...
            super::git2_utils::add_files(&repo, &[&format!("file{}.txt", i)])?;
            super::git2_utils::create_commit(&repo, &format!("Add file{}.txt", i), None, None)?;
        }
    ...
    fn test_get_last_commit_info() {
        ...
        let _ = &_temp_dir; // Keep temp dir alive for test duration
    ...
    fn test_get_branch_history() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::checkout_new_branch(&repo, "feature-branch", None).unwrap();
    ...
    fn test_get_commits_unique_to_branch() {
        ...
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::add_files(&repo, &["feature1.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add feature 1", None, None).unwrap();

        fs::write(temp_dir.path().join("feature2.txt"), "Feature 2").unwrap();
        let repo = git2::Repository::open(temp_dir.path()).unwrap();
        super::git2_utils::add_files(&repo, &["feature2.txt"]).unwrap();
        super::git2_utils::create_commit(&repo, "Add feature 2", None, None).unwrap();
    ...
    fn test_empty_repository_history() {
        ...
        // Initialize git repo but don't create any commits using libgit2
        let repo = git2::Repository::init(repo_path).unwrap();
        let mut config = repo.config().unwrap();
        config.set_str("user.name", "Test User").unwrap();
        config.set_str("user.email", "test@example.com").unwrap();
    ...
    #[test]
    fn test_commit_history_output_format() {
        let (_temp_dir, git_ops) = create_test_repo_with_commits().unwrap();

...
mod performance_tests {
    ...
    fn test_parsing_performance() {
        ...
        println!("Parsed {} config items in {:?}", config.len(), parse_duration);
    }
    ...
    fn test_large_file_performance() {
        ...
        println!("Loaded large config file ({} bytes) in {:?}", 
                large_content.len(), total_duration);
    }
    ...
    fn test_contains_key_performance() {
        ...
        let test_keys: Vec<String> = (0..100).map(|i| {
            format!("section_{}.subsection_{}.item_{}", i, i % 50, i % 10)
        }).collect();
    ...
    fn test_environment_substitution_performance() {
        ...
        assert_eq!(
            config.get("env_var_99").unwrap().coerce_to_string().unwrap(),
            "value_99"
        );
        
        println!("Performed environment substitution on 100 variables in {:?}", 
    ...
    fn test_concurrent_access_performance() {
        ...
        for thread_id in 0..4 {
            ...
            let handle = thread::spawn(move || {
                ...
                for i in 0..1000 {
                    let key = format!("section_{}.subsection_{}.item_{}", 
                                    i % 50, (i + thread_id) % 25, i % 10);
        ...
        println!("Performed {} concurrent accesses in {:?}", 
                total_accesses, concurrent_duration);
    }
...
"#, i, i, i, i % 2 == 0, i as f64));
            
            // Add array every 10 sections
            if i % 10 == 0 {
                content.push_str(&format!(r#"items = ["item_{}", "item_{}", "item_{}"]
"#, i, i + 1, i + 2));
            }
...
"#, i, i, i * 2));
    ...
    pub fn create_large_nested_config(sections: usize) -> Configuration {
        ...
        for i in 0..sections {
            for j in 0..10 {
                for k in 0..10 {
                    let key = format!("section_{}.subsection_{}.item_{}", i, j, k);
                    let value = ConfigValue::String(format!("value_{}_{}", i, j));
...
mod memory_tests {
    ...
    fn test_deep_nesting_memory() {
        ...
        assert!(deep_config.contains_key("level_0.level_1.level_2.level_3.level_4.level_5.level_6.level_7.value"));
    }
    ...
    fn test_large_array_memory() {
        ...
        let array = config.get("large_array").unwrap().coerce_to_array().unwrap();
    ...
    fn test_string_memory_efficiency() {
        ...
        for i in 0..10 {
            ...
            config.insert(format!("medium_{}", i), 
                         ConfigValue::String(medium_string));
        }
        ...
        assert!(config.validate().is_ok());
        assert_eq!(config.get("large").unwrap().coerce_to_string().unwrap().len(), 10000);
    }
    ...
    fn test_configuration_cloning() {
        ...
        modified_clone.insert("new_key".to_string(), ConfigValue::String("new_value".to_string()));
...
mod caching_tests {
    ...
    fn test_parser_reuse() {
        ...
        for (i, config_str) in configs.iter().enumerate() {
            ...
            assert_eq!(
                parsed_configs[i].get("name").unwrap().coerce_to_string().unwrap(),
                format!("Config{}", i + 1)
            );
        }
        ...
        println!("Parsed {} configs with reused parser in {:?}", 
                configs.len(), reuse_duration);
    }
    ...
    fn test_keys_caching_behavior() {
        ...
        // Add initial keys
        for i in 0..100 {
            config.insert(format!("initial_{}", i), ConfigValue::String(format!("value_{}", i)));
        }
        ...
        for i in 100..200 {
            config.insert(format!("added_{}", i), ConfigValue::String(format!("value_{}", i)));
        }
    ...
    fn test_validation_caching() {
        ...
        for i in 0..100 {
            config.insert(format!("valid_key_{}", i), ConfigValue::String(format!("value_{}", i)));
        }
        ...
        config.insert("123invalid".to_string(), ConfigValue::String("invalid key name".to_string()));
    ...
    fn test_environment_regex_caching() {
        ...
        for i in 0..100 {
            ...
            values.push(ConfigValue::String(format!("${{{}}}", format!("CACHE_TEST_{}", i))));
        }

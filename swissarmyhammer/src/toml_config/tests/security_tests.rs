...
fn test_file_size_limits() {
    ...
    if let Err(ConfigError::FileTooLarge { size, max_size: limit }) = result {
...
fn test_nesting_depth_limits() {
    ...
    assert!(result.is_ok(), "Configuration within depth limit should parse successfully");
    
    if let Ok(config) = result {
        let validation_result = config.validate();
        assert!(validation_result.is_ok(), "Configuration within depth limit should validate successfully");
    }
    ...
    deep_table.insert("value".to_string(), ConfigValue::String("deepest value".to_string()));
    ...
    assert!(validation_result.is_err(), "Deeply nested structure should fail validation");
    
    if let Err(ConfigError::NestingTooDeep { depth, max_depth: limit }) = validation_result {
        assert!(depth > limit, "Error should report correct depth comparison");
    } else {
    ...
    for attempt in traversal_attempts {
        ...
        match result {
            ...
            Err(other) => {
                ...
                println!("Path traversal attempt '{}' rejected with: {:?}", attempt, other);
            }
        }
    }
...
fn test_malformed_toml_handling() {
    ...
    for (input, description) in malformed_inputs {
        ...
        match result {
            ...
            Err(ConfigError::TomlParseGeneric(_)) => {
                // Expected error types
            }
            Err(other) => {
                println!("Malformed TOML '{}' produced unexpected error: {:?}", description, other);
                // Still acceptable as long as it fails
            }
            Ok(_) => {
                panic!("Malformed TOML '{}' should not parse successfully", description);
            }
        }
    }
}
...
fn test_invalid_environment_variable_syntax() {
    ...
    for (invalid_syntax, description) in invalid_env_vars {
        ...
        assert!(result.is_ok(), "TOML with invalid env var syntax should still parse");
        ...
        if let Ok(mut config) = result {
            ...
            match substitution_result {
                Ok(_) => {
                    ...
                    if let Some(ConfigValue::String(value)) = config.get("test_var") {
                        assert_eq!(
                            ...
                            "Invalid env var syntax '{}' should be left unchanged",
                            description
                        );
                    }
                }
                Err(_) => {
                    ...
                    println!("Invalid env var syntax '{}' rejected during substitution", description);
                }
            }
        }
    }
...
fn test_variable_name_validation() {
    ...
    let reserved_names = vec![
        "for", "if", "unless", "case", "when", "else", "endif",
        "endfor", "endunless", "endcase", "break", "continue",
        "assign", "capture", "include", "layout", "raw", "endraw",
        "comment", "endcomment",
    ];
    ...
    for name in reserved_names {
        ...
        assert!(
            result.is_err(),
            "Reserved variable name '{}' should be invalid",
            name
        );
        config.remove(name); // Clean up for next test
    }
...
fn test_string_length_validation() {
    ...
    if let Err(ConfigError::StringTooLarge { size, max_size }) = result {
        assert!(size > max_size, "Error should report correct size comparison");
    } else {
        ...
        match result {
            Ok(config) => {
                ...
                if validation_result.is_err() {
                    println!("Potentially dangerous content '{}' rejected during validation", key);
                } else {
            ...
            Err(_) => {
                println!("Potentially dangerous content '{}' rejected during parsing", key);
...
fn test_memory_safety_large_configurations() {
    ...
    for i in 0..num_keys {
        large_config.push_str(&format!("key_{} = \"value_{}\"\n", i, i));
    }

...
name: pollution-test-workflow-a
title: Pollution Test Workflow A
description: Parent workflow with states 1, 2, 3
---

# Pollution Test Workflow A
...
- 2: Run workflow "pollution-test-workflow-b" with result="sub_result"
...
name: pollution-test-workflow-b
title: Pollution Test Workflow B
description: Child workflow with states 1, 2, 3
---

# Pollution Test Workflow B
...
"#;
    ...
    let storage = setup_test_storage_with_workflows(&[
        ("pollution-test-workflow-a", parent_workflow_content),
        ("pollution-test-workflow-b", child_workflow_content),
    ]);
...
async fn test_nested_workflow_correct_action_execution() {
    ...
    let parent_workflow_content = r#"---
name: action-exec-test-workflow-parent
title: Action Exec Test Parent Workflow
description: Parent workflow that calls child workflow
---

# Action Exec Test Parent Workflow
...
- CallChild: Run workflow "action-exec-test-workflow-child" with parent_log="${execution_log}" result="child_result"
- Verify: Set execution_log="${execution_log},parent:Verify"
"#;

    let child_workflow_content = r#"---
name: action-exec-test-workflow-child
title: Action Exec Test Child Workflow
description: Child workflow with potentially conflicting state names
---

# Action Exec Test Child Workflow
...
"#;
    ...
    // Set up test storage with workflows
    let storage = setup_test_storage_with_workflows(&[
        ("action-exec-test-workflow-parent", parent_workflow_content),
        ("action-exec-test-workflow-child", child_workflow_content),
    ]);
    set_test_storage(storage);
...
name: nested-isolation-test-workflow-level-a
title: Nested Isolation Test Level A Workflow
description: Top level workflow
---

# Nested Isolation Test Level A Workflow
...
- CallB: Run workflow "nested-isolation-test-workflow-level-b" with a_data="${level_a_data}" result="b_result"
- FinalA: Set final_a="${level_a_data},${b_result}"
"#;

    let workflow_b_content = r#"---
name: nested-isolation-test-workflow-level-b
title: Nested Isolation Test Level B Workflow
description: Middle level workflow
---

# Nested Isolation Test Level B Workflow
...
- CallC: Run workflow "nested-isolation-test-workflow-level-c" with b_data="${level_b_data}" result="c_result"
- FinalB: Set final_b="${level_b_data},${c_result}"
"#;

    let workflow_c_content = r#"---
name: nested-isolation-test-workflow-level-c
title: Nested Isolation Test Level C Workflow
description: Deepest level workflow
---

# Nested Isolation Test Level C Workflow
...
"#;
    ...
    // Set up test storage with workflows
    let storage = setup_test_storage_with_workflows(&[
        ("nested-isolation-test-workflow-level-a", workflow_a_content),
        ("nested-isolation-test-workflow-level-b", workflow_b_content),
        ("nested-isolation-test-workflow-level-c", workflow_c_content),
    ]);
    set_test_storage(storage);

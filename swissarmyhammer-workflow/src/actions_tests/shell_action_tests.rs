...
fn test_environment_variable_name_validation() {
    use crate::actions::is_valid_env_var_name;
...
fn test_working_directory_validation() {
    use crate::actions::validate_working_directory;
...
fn test_command_validation_empty_command() {
    use crate::actions::validate_command;
...
fn test_working_directory_security_validation() {
    use crate::actions::validate_working_directory_security;
...
fn test_environment_variables_security_validation() {
    use crate::actions::validate_environment_variables_security;
    use std::collections::HashMap;
    ...
    for cmd in &dangerous_commands {
        ...
        if cmd == &"echo 'safe command'" {
            assert!(
                ...
                "Safe command '{cmd}' should not be blocked"
            );
        }
        // Note: Other commands may be blocked by the security framework which is expected behavior
    }
}
...
mod comprehensive_parser_tests {
    ...
    fn test_parse_shell_action_whitespace_handling() {
        dbg!("Test starting");
        let parser = ActionParser::new().unwrap();
        dbg!("Parser created successfully");

        // Test case 1: Leading/trailing spaces should be trimmed, multiple internal spaces should work
        ...
        if let Some(captures) = shell_regex.captures(trimmed) {
            ...
            for (i, cap) in captures.iter().enumerate() {
                if let Some(m) = cap {
                    println!("DEBUG:   Capture {}: {:?}", i, m.as_str());
                }
            }
        }
        ...
        let option1 = result1.unwrap();
        assert!(option1.is_some(), "First test should return Some(action)");
        let action = option1.unwrap();
        assert_eq!(action.command, "echo hello");

        // Test the second case
        let result2 = parser.parse_shell_action("Shell\t\"echo hello\"");
        ...
        // Test the third case
        let result3 = parser.parse_shell_action("Shell \"echo hello\" with  timeout=30");
        assert!(result3.is_ok(), "Third test should parse without error");
        let option3 = result3.unwrap();
        assert!(option3.is_some(), "Third test should return Some(action)");
        let action = option3.unwrap();
        assert_eq!(action.command, "echo hello");
...
mod timeout_and_process_management_tests {
    ...
    async fn test_concurrent_timeout_commands() {
        ...
        let mut context1 = WorkflowTemplateContext::with_vars_for_test(HashMap::new());
        let mut context2 = WorkflowTemplateContext::with_vars_for_test(HashMap::new());
...
mod integration_tests {
    use super::*;
    use crate::actions::parse_action_from_description;
    ...
    fn test_action_any_trait_integration() {
        let action = ShellAction::new("echo test".to_string());
        let action_trait: Box<dyn crate::actions::Action> = Box::new(action);

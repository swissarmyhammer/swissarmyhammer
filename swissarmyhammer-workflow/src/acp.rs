...
//!
//! This module provides direct integration with ACP agents (claude-agent and llama-agent)
//! for executing prompts within workflows.
//!
...
pub struct McpServerConfig {
    /// URL of the MCP server (e.g., "http://localhost:8080/sse")
    pub url: String,
}

impl McpServerConfig {
    /// Create MCP server config from port number
...
async fn create_claude_agent(mcp_config: Option<McpServerConfig>) -> AcpResult<AcpAgentHandle> {
    ...
    if let Some(mcp) = mcp_config {
        agent_config.mcp_servers = vec![claude_agent::config::McpServerConfig::Sse(
            claude_agent::config::SseTransport {

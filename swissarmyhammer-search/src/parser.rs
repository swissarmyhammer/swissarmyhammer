...
impl ParserConfig {
    ...
    pub fn new(
        ...
        if min_chunk_size > max_chunk_size {
            return Err(SearchError::TreeSitter(format!(
        ...
        if min_chunk_size == 0 {
            return Err(SearchError::TreeSitter(
...
impl CodeParser {
    ...
    pub fn new(config: ParserConfig) -> SearchResult<Self> {
    ...
    pub fn parse_file(&self, file_path: &Path, content: &str) -> SearchResult<Vec<CodeChunk>> {
    ...
    fn parse_with_treesitter(
        ...
        if !self.has_parser_for_language(language) {
            ...
            return Err(SearchError::TreeSitter(error_msg));
        }
        ...
        let tree = {
            let mut parser_ref = self.parsers.get_mut(language).ok_or_else(|| {
                SearchError::TreeSitter(format!("Parser disappeared for language: {language:?}"))
            })?;
            ...
            parser_ref.parse(content, None)
                .ok_or_else(|| {
                    ...
                    SearchError::TreeSitter(error_msg)
                })?
        };
    ...
    fn parse_as_plain_text(&self, file_path: &Path, content: &str) -> SearchResult<Vec<CodeChunk>> {
        let content_hash = FileHasher::hash_string(content);
        let language = self.detect_language(file_path);
    ...
    ///
    /// # Errors
    /// Returns `SearchError::TreeSitter` with query validation details if any query is invalid
    ...
    /// # Returns
    /// `Ok(())` if all queries are valid, or error with details of first invalid query
    fn validate_all_queries(&self) -> SearchResult<()> {
    ...
    pub fn add_language_support(&mut self, definition: LanguageDefinition) -> SearchResult<()> {
        ...
        if let Some(parser) = Self::create_parser_for_language(&definition) {
            ...
            Err(SearchError::TreeSitter(format!(

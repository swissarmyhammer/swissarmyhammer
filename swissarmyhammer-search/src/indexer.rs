...
impl FileIndexer {
    ...
    pub async fn new(storage: VectorStorage) -> SearchResult<Self> {
    ...
    async fn filter_changed_files(&self, paths: Vec<PathBuf>) -> SearchResult<Vec<PathBuf>> {
        // Use the dedicated change tracker to check for file changes
        let change_report = self
            .change_tracker
            .check_files_for_changes(paths)
            .map_err(|e| SearchError::Index(format!("Failed to check files for changes: {e}")))?;
    ...
    async fn index_single_file(
        ...
        use crate::parser::DEFAULT_MAX_FILE_SIZE_BYTES;
    ...
    pub async fn incremental_index(&mut self, pattern: &str) -> SearchResult<IndexingReport> {
        self.index_glob(pattern, false).await
    }

    /// Force re-index all files matching pattern
    pub async fn full_reindex(&mut self, pattern: &str) -> SearchResult<IndexingReport> {
...
pub struct IndexingReport {
    ...
    pub errors: Vec<(PathBuf, crate::error::SearchError)>,
...
impl IndexingReport {
    ...
    pub fn add_error(&mut self, file_path: PathBuf, error: crate::error::SearchError) {
        self.files_processed += 1;
        self.files_failed += 1;
...
mod tests {
    ...
    async fn create_test_indexer() -> SearchResult<(FileIndexer, TempDir)> {
        let temp_dir = TempDir::new().map_err(SearchError::FileSystem)?;

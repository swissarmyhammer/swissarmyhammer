...
impl VectorStorage {
    ...
    pub fn new(config: SemanticConfig) -> SearchResult<Self> {
    ...
    pub fn initialize(&self) -> SearchResult<()> {
        ...
        conn.execute(Self::CREATE_INDEXED_FILES_TABLE, [])
            .map_err(|e| {
                SearchError::Storage(format!("Failed to create indexed_files table: {e}"))
            })?;

        // Create code_chunks table
        conn.execute(Self::CREATE_CODE_CHUNKS_TABLE, [])
            .map_err(|e| {
                SearchError::Storage(format!("Failed to create code_chunks table: {e}"))
            })?;
    ...
    pub fn store_indexed_file(&self, file: &IndexedFile) -> SearchResult<()> {
        ...
        .map_err(|e| SearchError::Storage(format!("Failed to store indexed file: {e}")))?;
    ...
    pub fn store_chunk(&self, chunk: &CodeChunk) -> SearchResult<()> {
        ...
        .map_err(|e| SearchError::Storage(format!("Failed to store chunk: {e}")))?;
    ...
    pub fn store_embedding(&self, embedding: &Embedding) -> SearchResult<()> {
        ...
        .map_err(|e| SearchError::Storage(format!("Failed to store embedding: {e}")))?;
    ...
    pub fn similarity_search(
        ...
        let mut stmt = conn.prepare(
            ...
            SearchError::Storage(format!("Failed to prepare similarity search query: {e}"))
        })?;
    ...
    pub fn similarity_search_with_details(
        ...
        limit: usize,
        threshold: f32,
    ) -> SearchResult<Vec<(String, f32, Vec<f32>)>> {
        tracing::debug!(
        ...
        for row_result in rows {
            let row_data = row_result.map_err(|e| {
                SearchError::Storage(format!("Failed to process detailed search row: {e}"))
            })?;
    ...
    pub fn get_chunk(&self, chunk_id: &str) -> SearchResult<Option<CodeChunk>> {
        ...
        let mut stmt = conn.prepare(
            ...
            SearchError::Storage(format!("Failed to prepare get_chunk query: {e}"))
        })?;
    ...
    pub fn get_all_chunks(&self) -> SearchResult<std::collections::HashMap<String, CodeChunk>> {
        ...
        let mut stmt = conn.prepare(
            ...
            SearchError::Storage(format!("Failed to prepare get_all_chunks query: {e}"))
        })?;
    ...
    pub fn get_file_chunks(&self, file_path: &Path) -> SearchResult<Vec<CodeChunk>> {
        ...
        let mut stmt = conn.prepare(
            ...
            SearchError::Storage(format!("Failed to prepare get_file_chunks query: {e}"))
        })?;
    ...
    pub fn remove_file(&self, file_path: &Path) -> SearchResult<()> {
        ...
        conn.execute(
            ...
            SearchError::Storage(format!("Failed to remove embeddings for file: {e}"))
        })?;
        ...
        .map_err(|e| {
            SearchError::Storage(format!("Failed to remove indexed file metadata: {e}"))
        })?;
    ...
    pub fn get_stats(&self) -> SearchResult<StorageStats> {
        ...
        let total_files: usize = conn
            ...
            .map_err(|e| SearchError::Storage(format!("Failed to get file count: {e}")))?;
    ...
    pub fn get_chunks_by_language(&self, language: &Language) -> SearchResult<Vec<CodeChunk>> {
        ...
        let mut stmt = conn.prepare(
            ...
            SearchError::Storage(format!("Failed to prepare get_chunks_by_language query: {e}"))
        })?;
        ...
        for row_result in rows {
            let chunk = row_result.map_err(|e| {
                SearchError::Storage(format!("Failed to parse chunk by language: {e}"))
            })?;
    ...
    pub fn maintenance(&self) -> SearchResult<()> {
        ...
        conn.execute("VACUUM", [])
            .map_err(|e| SearchError::Storage(format!("Failed to run VACUUM: {e}")))?;

        // Perform ANALYZE to update table statistics for query optimization
        tracing::debug!("Running ANALYZE operation");
        conn.execute("ANALYZE", [])
            .map_err(|e| SearchError::Storage(format!("Failed to run ANALYZE: {e}")))?;
        ...
        for table in &["indexed_files", "code_chunks", "embeddings"] {
            ...
            conn.execute(&analyze_sql, []).map_err(|e| {
                SearchError::Storage(format!("Failed to analyze table {table}: {e}"))
            })?;
        }
    ...
    pub fn file_exists(&self, file_path: &Path) -> SearchResult<bool> {
    ...
    pub fn get_file_hash(&self, file_path: &Path) -> SearchResult<Option<ContentHash>> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| {
                SearchError::Storage(format!("Failed to prepare get_file_hash query: {e}"))
            })?;
        ...
        let mut rows = stmt
            ...
            .map_err(|e| {
                SearchError::Storage(format!("Failed to execute get_file_hash query: {e}"))
            })?;
    ...
    pub fn store_indexed_file_internal(&self, file: &IndexedFile) -> SearchResult<()> {
    ...
    pub fn store_chunks_and_embeddings_transaction(
        ...
        conn.execute("COMMIT", []).map_err(|e| {
            ...
            let _ = conn.execute("ROLLBACK", []);
            SearchError::Storage(format!("Failed to commit transaction: {e}"))
        })?;
    ...
    fn parse_chunk_type(s: &str) -> crate::types::ChunkType {
        match s {
            ...
            "Import" => crate::types::ChunkType::Import,
            _ => crate::types::ChunkType::PlainText,
        }
    }
...
mod tests {
    use super::*;
    use crate::types::{ChunkType, FileId};
    use chrono::Utc;

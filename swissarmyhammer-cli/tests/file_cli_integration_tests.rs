...
fn test_file_read_basic_functionality() -> Result<()> {
    ...
    assert!(stdout.contains(test_content), "Output should contain file content");
    assert!(stderr.is_empty() || !stderr.contains("error"), "Should not have errors");
...
fn test_file_read_with_offset_and_limit() -> Result<()> {
    ...
    assert_eq!(exit_code, Some(0), "Command should succeed");
    assert!(!stdout.contains("Line 1\n") && !stdout.starts_with("Line 1"), "Should not contain line 1 when starting from line 6");
...
fn test_file_write_basic_functionality() -> Result<()> {
    ...
    assert_eq!(written_content, test_content, "Written content should match");
...
fn test_file_write_overwrite_existing() -> Result<()> {
    ...
    assert_ne!(final_content, initial_content, "Old content should be replaced");
...
fn test_file_write_creates_parent_directories() -> Result<()> {
    ...
    assert_eq!(exit_code, Some(0), "Command should succeed");
    assert!(test_file.exists(), "File should be created");
    assert!(test_file.parent().unwrap().exists(), "Parent directories should be created");
...
fn test_file_edit_basic_replacement() -> Result<()> {
    ...
    assert!(edited_content.contains("new_value"), "Should contain replacement text");
    assert!(edited_content.contains("old_value"), "Should still contain one instance (only first replaced)");
...
fn test_file_edit_replace_all() -> Result<()> {
    ...
    let edited_content = fs::read_to_string(&test_file)?;
    assert!(!edited_content.contains("TARGET"), "Should not contain original text");
    assert_eq!(edited_content.matches("RESULT").count(), 3, "Should replace all instances");
...
fn test_file_edit_string_not_found() -> Result<()> {
    ...
    let final_content = fs::read_to_string(&test_file)?;
    assert_eq!(final_content, initial_content, "Content should be unchanged");
...
fn test_file_glob_basic_patterns() -> Result<()> {
    ...
    assert_eq!(exit_code, Some(0), "Command should succeed");
    assert!(stdout.contains("file1.txt"), "Should find top-level txt files");
...
fn test_file_glob_case_sensitivity() -> Result<()> {
    ...
    assert!(!stdout.contains("README.TXT"), "Should not find different case");
...
fn test_file_grep_basic_search() -> Result<()> {
    ...
    let test_files = vec![
        ("search1.txt", "This file contains TARGET_STRING for testing."),
...
fn test_complete_file_workflow() -> Result<()> {
    ...
    assert!(stdout.contains("OLD_VALUE"), "Should contain original content");
    ...
    assert!(stdout.contains("NEW_VALUE"), "Should contain edited content");
    assert!(!stdout.contains("OLD_VALUE"), "Should not contain old content");
...
fn test_file_discovery_and_search_workflow() -> Result<()> {
    ...
    let test_files = vec![
        ...
        ("src/lib.rs", "pub fn process_TARGET() -> String {\n    \"result\".to_string()\n}"),
        ("tests/test.rs", "#[test]\nfn test_TARGET() {\n    assert!(true);\n}"),
...
fn test_output_formatting_consistency() -> Result<()> {
    ...
    assert!(stdout.contains(content) || stdout.contains("success"), "Should be meaningful");
...
fn test_help_command_functionality() -> Result<()> {
    ...
    assert!(stdout.contains("read") || stdout.contains("write"), "Should show subcommands");
    ...
    assert!(stdout.contains("offset") || stdout.contains("limit"), "Should show read options");

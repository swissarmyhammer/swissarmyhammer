...
async fn test_cli_generation_with_real_registry() {
    ...
    assert!(registry.len() > 0, "Registry should contain registered tools");
    ...
    assert!(!commands.is_empty(), "Should generate at least some commands");
    ...
    for command in &commands {
        ...
        assert!(!command.name.contains('_'), "Command name should use dashes: {}", command.name);
        ...
        for arg in &command.arguments {
            if !arg.required && !found_optional {
                ...
                panic!("Required arguments should come before optional ones in command: {}", command.name);
            }
        }
    }
...
async fn test_naming_strategies() {
    ...
    if domain_commands.len() > 1 {
        assert!(has_parents || has_subcommands, "GroupByDomain should create hierarchical structure");
    }
...
async fn test_cli_exclusion_respect() {
    ...
    let eligible_tools: Vec<_> = registry.get_cli_eligible_tools().into_iter()
        .map(|meta| meta.name.clone())
        .collect();
    
    println!("All tools: {} | CLI-eligible: {}", all_tools.len(), eligible_tools.len());
    ...
    if all_tools.len() > eligible_tools.len() {
        ...
        for tool_name in &excluded_tools {
            if all_tools.contains(&tool_name.to_string()) {
                assert!(!eligible_tools.contains(&tool_name.to_string()), 
                    "Tool '{}' should be CLI-excluded", tool_name);
                println!("✅ Confirmed {} is excluded as expected", tool_name);
            }
        }
    }
    ...
    for excluded in &["issue_work", "issue_merge", "abort_create"] {
        ...
        assert!(!command_tool_names.contains(&&excluded_string), 
            "Excluded tool '{}' should not generate a command", excluded);
    }
...
async fn test_schema_parsing_comprehensive() {
    ...
    for command in &commands {
        ...
        for arg in &command.arguments {
            assert!(!arg.name.contains('_'), "Argument names should use dashes: {}", arg.name);
        }
        
        for option in &command.options {
            assert!(!option.name.contains('_'), "Option names should use dashes: {}", option.name);
            assert!(option.long.starts_with("--"), "Long options should start with --: {}", option.long);
        }
    }
...
async fn test_generation_error_handling() {
    ...
    let result = generator.generate_commands();
    assert!(result.is_ok(), "Empty registry should succeed with empty commands list");
    assert!(result.unwrap().is_empty(), "Empty registry should generate no commands");
    ...
    if registry.len() > 1 {
        ...
        match result {
            Ok(commands) => {
                ...
                println!("✅ Command limit respected (generated {} commands)", commands.len());
            }
            Err(e) => {

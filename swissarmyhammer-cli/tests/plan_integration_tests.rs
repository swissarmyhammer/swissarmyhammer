...
"#;
    ...
    for issue_file in &issue_files {
        ...
        assert!(
            ...
            "Issue file {issue_file} should reference the plan file: {content}"
        );
    }
...
"#;
    ...
    assert!(
        issue_files.len() >= 5,
        "Complex specification should create multiple issue files. Found: {issue_files:?}"
    );
...
async fn test_plan_command_with_existing_issues() -> Result<()> {
    ...
    let issues_dir = temp_path.join("issues");
    fs::write(
        issues_dir.join("EXISTING_000001_old-issue.md"),
        "# Old Issue",
    )?;
    fs::write(
        issues_dir.join("OTHER_000001_different-project.md"),
        "# Different Project",
    )?;
...
"#;
    ...
    assert!(
        issues_dir
            .join("OTHER_000001_different-project.md")
            .exists(),
...
async fn test_plan_command_directory_instead_of_file() -> Result<()> {
    ...
    assert!(
        ...
        "Should show directory error: {stderr}"
    );
...
"#;
    ...
    assert!(
        !issue_files.is_empty(),
        "Should create issue files with relative path"
    );
...
"#;
    ...
    assert!(
        !issue_files.is_empty(),
        "Should create issue files with absolute path"
    );
...
"#;
    ...
    for issue_file in &issue_files {
        ...
        assert!(
            ...
            "Issue file should contain meaningful content: {issue_file}"
        );
...
async fn test_plan_command_large_file() -> Result<()> {
    ...
    for i in 1..=20 {
        ...
        large_content.push_str(&format!(
            "This is section {i} with detailed requirements.\n\n"
        ));
        large_content.push_str("### Requirements\n");
        for j in 1..=5 {
            large_content.push_str(&format!("{j}. Requirement {j} for section {i}\n"));
        }
        large_content.push('\n');
    }
    ...
    if !output.status.success() {
        ...
        assert!(
            ...
            "Should handle large files gracefully: {stderr}"
        );
    } else {
...
"#;
    ...
    assert!(
        duration.as_secs() < 60,
        "Plan command should complete within reasonable time: {duration:?}"
    );
...
"#;
    ...
    if cfg!(unix) {
        ...
        assert!(
            ...
            "Should show permission error: {stderr}"
        );
...
"#;
    ...
    assert!(
        !issue_files.is_empty(),
        "Should create issue files in git repo"
    );
...
"#;
    ...
    assert!(
        !issue_files.is_empty(),
        "Should create issue files with Unicode content"
    );
...
"#;
    ...
    match output {
        Ok(_result) => {
            ...
            // Plan completed within short timeout - this is acceptable
        }
        Err(e) if e.to_string().contains("timeout") => {
            // Timeout is expected and acceptable
            // Plan command handled timeout as expected - this is acceptable
        }
        Err(e) => {
            return Err(e.into());
        }
...
"#;
    ...
    assert!(
        !issue_files.is_empty(),
        "Should create issue files despite malformed markdown"
    );

...
fn create_nested_agent_structure(base_dir: &Path, depth: usize, agents_per_level: usize) -> Result<()> {
    fn create_level(current_dir: &Path, remaining_depth: usize, agents_per_level: usize, level_index: usize) -> Result<()> {
...
executor:
  ...
  config:
    ...
    args: ["--nested-level-{}"]
quiet: false"#, level_index, i, level_index, i, level_index, i, level_index);

                fs::write(agents_dir.join(format!("{}.yaml", agent_name)), agent_content)?;
            }
        }
        
        // Create subdirectories if we haven't reached max depth
        if remaining_depth > 1 {
            for i in 0..2 { // Create 2 subdirs per level to keep it manageable
                let subdir = current_dir.join(format!("level-{}-sub-{}", level_index, i));
                create_level(&subdir, remaining_depth - 1, agents_per_level, level_index + 1)?;
            }
        }
...
fn create_invalid_agent_files(dir: &Path) -> Result<Vec<String>> {
    ...
    let syntax_errors = vec![
        ...
        ("invalid-indentation.yaml", "description: test\n  invalid_indent: true\nwrong_level: false"),
        ("mixed-tabs-spaces.yaml", "description: test\n\ttabbed: true\n    spaced: true"),
        ("duplicate-keys.yaml", "description: first\ndescription: duplicate\nexecutor:\n  type: claude-code"),
    ];
    ...
    let structure_errors = vec![
        ("missing-executor.yaml", r#"---
description: "Missing executor section"
---
quiet: false"#),
        ("invalid-executor-type.yaml", r#"---
...
async fn test_large_agent_list_performance() -> Result<()> {
    ...
    let list_output = run_sah_command_with_timeout(&["agent", "list", "--format", "json"], 
                                                  Some(&project_root), 30).await?;
    
    let duration = start_time.elapsed();
    
    assert!(list_output.status.success(), "Should list large number of agents");
    assert!(duration < Duration::from_secs(10), 
            "Should list {} agents in under 10 seconds, took {:?}", 
            user_agents.len() + project_agents.len() + 3, duration); // +3 for builtins
    ...
    assert!(agents_array.len() >= expected_min,
            "Should list at least {} agents, got {}", expected_min, agents_array.len());
...
"#, i, i, i, i, i * 2, i % 2 == 0));
    ...
    let use_output = run_sah_command_with_timeout(&["agent", "use", "claude-code"], 
                                                 Some(project_root), 15).await?;
    
    let duration = start_time.elapsed();
    
    if use_output.status.success() {
        assert!(duration < Duration::from_secs(5), 
                "Should update large config in under 5 seconds, took {:?}", duration);
        
        // Verify all data is preserved
        let updated_config = fs::read_to_string(&config_path)?;
        assert!(updated_config.contains("prompts:"), "Should preserve prompts");
        assert!(updated_config.contains("workflows:"), "Should preserve workflows");
        assert!(updated_config.contains("workflow_499:"), "Should preserve last workflow");
        assert!(updated_config.contains("other_data:"), "Should preserve other data");
        assert!(updated_config.contains("key_199:"), "Should preserve custom data");
        assert!(updated_config.contains("agent:"), "Should add agent section");
        
        println!("Updated large config ({} bytes) in {:?}", 
                updated_config.len(), duration);
    }
...
async fn test_deeply_nested_project_structures() -> Result<()> {
    ...
    let test_dirs = vec![
        ...
        base_dir.join("level-0-sub-1").join("level-1-sub-0").join("level-2-sub-1"),
    ];
    
    for test_dir in test_dirs {
        if test_dir.exists() {
            let list_output = run_sah_command_with_timeout(&["agent", "list"], 
                                                          Some(&test_dir), 10).await?;
            
            if list_output.status.success() {
                let stdout = String::from_utf8_lossy(&list_output.stdout);
                // Should find project agents from current level
                assert!(stdout.contains("Project:") || stdout.contains("project"),
                        "Should find project agents from nested dir: {:?}", test_dir);
            }
        }
    }
...
async fn test_deep_directory_traversal_performance() -> Result<()> {
    ...
    if deep_dir.exists() {
        ...
        let list_output = run_sah_command_with_timeout(&["agent", "list"], 
                                                      Some(&deep_dir), 15).await?;
        
        let duration = start_time.elapsed();
        
        assert!(list_output.status.success() || duration < Duration::from_secs(10),
                "Should handle deep directory structure efficiently, took {:?}", duration);
...
async fn test_invalid_yaml_recovery() -> Result<()> {
    ...
    assert!(agents_array.len() >= valid_agents.len() + 3, // +3 for builtins
            "Should load {} valid agents, got {}", valid_agents.len() + 3, agents_array.len());
    
    // Verify valid agents are present
    let agent_names: Vec<_> = agents_array.iter()
        .filter_map(|a| a["name"].as_str())
        .collect();
    
    for valid_agent in &valid_agents {
        assert!(agent_names.contains(&valid_agent.as_str()),
                "Should include valid agent: {}", valid_agent);
    }
    
    // Verify invalid files are not loaded
    for invalid_file in &invalid_files {
        let invalid_name = invalid_file.replace(".yaml", "");
        assert!(!agent_names.contains(&invalid_name.as_str()),
                "Should not load invalid agent: {}", invalid_name);
    }
    ...
    if !use_output.status.success() {
        ...
        assert!(!stderr.contains("parse") && !stderr.contains("yaml") && 
                !stderr.contains("syntax"),
                "Should not fail with YAML parsing errors: {}", stderr);
    }
...
"#;
    ...
    if !use_output.status.success() {
        ...
        assert!(stderr.contains("configuration") || stderr.contains("config") || 
                ...
                "Should provide helpful error message: {}", stderr);
    } else {
        ...
        assert!(parsed.get("agent").is_some(), "Should have valid agent section");
    }
...
async fn test_rapid_sequential_operations() -> Result<()> {
    ...
    let agents = ["claude-code", "qwen-coder", "claude-code", "qwen-coder-flash", "claude-code"];
    ...
    for (i, agent) in agents.iter().enumerate() {
        ...
        let list_output = run_sah_command_with_timeout(&["agent", "list", "--format", "json"], 
                                                      Some(project_root), 5).await?;
        
        let use_output = run_sah_command_with_timeout(&["agent", "use", agent], 
                                                     Some(project_root), 5).await?;
        
        let operation_time = start_time.elapsed();
        
        // Operations should complete quickly
        assert!(operation_time < Duration::from_secs(3),
                "Rapid operation {} should complete quickly, took {:?}", i, operation_time);
        
        // At least list should succeed
        assert!(list_output.status.success(), 
                "List should succeed on rapid operation {}", i);
        
        // If use succeeded, verify config consistency
        if use_output.status.success() && config_path.exists() {
            let config_content = fs::read_to_string(&config_path)?;
            // Should be valid YAML
            let _: serde_yaml::Value = serde_yaml::from_str(&config_content)
                .map_err(|e| anyhow::anyhow!("Config corrupted after rapid operation {}: {}", i, e))?;
        }
    }
...
"#;
    ...
    for i in 0..5 {
        let agent = if i % 2 == 0 { "claude-code" } else { "qwen-coder" };
        ...
        if !use_output.status.success() {
            ...
            assert!(!stderr.contains("Permission denied") || 
                    stderr.contains("configuration") || stderr.contains("write"),
                    "Should provide user-friendly error for file issues: {}", stderr);
        }
        ...
        if config_path.exists() {
            ...
            if !config_content.is_empty() {
                let _: serde_yaml::Value = serde_yaml::from_str(&config_content)
                    .map_err(|e| anyhow::anyhow!("Config integrity lost after operation {}: {}", i, e))?;
            }
        }
    }
...
async fn test_memory_usage_with_large_datasets() -> Result<()> {
    ...
    assert!(duration < Duration::from_secs(30), 
            ...
            user_agents.len() + project_agents.len() + 3, duration);
    ...
    let expected_min = (user_agents.len() + project_agents.len()) / 2 + 3; // At least half + builtins
    assert!(agents_array.len() >= expected_min,
            "Should load at least {} agents, got {}", expected_min, agents_array.len());
...
entry_{:05}:
  ...
  data:
    ...
    nested_data:
      ...
      sub_b: {}
      sub_c: "deep_nested_{}"
"#, i, i, i, i * 2, i % 3 == 0, i, i, i * 3, i));
    }
    
    fs::write(&config_path, &huge_config)?;
    
    let file_size = huge_config.len();
    println!("Created config file of {} bytes ({:.2} MB)", file_size, file_size as f64 / 1024.0 / 1024.0);
    ...
    if use_output.status.success() {
        ...
        let updated_config = fs::read_to_string(&config_path)?;
        assert!(updated_config.contains("entry_04999:"), "Should preserve all entries");
        assert!(updated_config.contains("agent:"), "Should add agent section");
        ...
        println!("Updated and validated {:.2} MB config in {:?} + {:?}", 
                file_size as f64 / 1024.0 / 1024.0, duration, parse_time);
    }
    ...
    for (invalid_agent, expected_error) in error_scenarios {
        ...
        if !valid_use.status.success() {
            ...
            assert!(!stderr.contains("not found"),
                    "Should not fail with 'not found' for builtin after recovery from {}", invalid_agent);
        }
    }

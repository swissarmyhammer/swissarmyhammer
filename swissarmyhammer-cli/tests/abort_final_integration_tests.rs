...
fn test_large_abort_reasons() -> Result<()> {
    ...
    assert!(!output.status.success(), "Workflow should fail with large abort reason");
...
fn test_unicode_abort_reasons() -> Result<()> {
    ...
    assert!(!output.status.success(), "Workflow should fail with unicode abort reason");
...
fn test_abort_file_cleanup_between_runs() -> Result<()> {
    ...
    assert!(!output1.status.success(), "First run should fail due to abort");
...
fn test_abort_with_different_cli_commands() -> Result<()> {
    ...
    println!("Prompt list with abort file - exit code: {:?}", output.status.code());
    ...
    assert!(!output2.status.success(), "Should fail for non-existent workflow");
...
fn test_cross_platform_abort_file_paths() -> Result<()> {
    ...
    assert!(abort_file.exists(), "Abort file should exist at expected path");

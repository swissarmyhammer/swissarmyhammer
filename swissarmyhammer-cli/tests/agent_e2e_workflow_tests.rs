...
fn find_agent_in_json<'a>(
    agents_json: &'a serde_json::Value,
    name: &str,
...
async fn test_basic_list_use_workflow() -> Result<()> {
    ...
    let list_output =
        run_sah_command(&["agent", "list", "--format", "json"], Some(project_root)).await?;
    ...
    assert!(
        find_agent_in_json(&agents_json, "claude-code").is_some(),
        "Should list claude-code agent"
    );
    assert!(
        find_agent_in_json(&agents_json, "qwen-coder").is_some(),
        "Should list qwen-coder agent"
    );
    assert!(
        find_agent_in_json(&agents_json, "qwen-coder-flash").is_some(),
    ...
    if use_output.status.success() {
        ...
        assert!(
            verify_agent_config(&config_path, "claude-code")?,
            "Config should contain claude-code agent"
        );

        // Step 4: Switch to different agent
        let switch_output =
            run_sah_command(&["agent", "use", "qwen-coder"], Some(project_root)).await?;

        if switch_output.status.success() {
            // Step 5: Verify config was updated
            assert!(
                verify_agent_config(&config_path, "qwen-coder")?,
...
async fn test_all_builtin_agents_workflow() -> Result<()> {
    ...
    for (i, agent_name) in builtin_agents.iter().enumerate() {
        ...
        if use_output.status.success() {
            // Verify config file
            assert!(
                verify_agent_config(&config_path, agent_name)?,
                "Config should contain {} after step {}",
                agent_name,
                i + 1
            );
            ...
            assert!(
                ...
                "Should be able to list agents after using {}",
                agent_name
            );

            let agents_json = parse_agent_list_json(&String::from_utf8_lossy(&list_output.stdout))?;
            assert!(
                find_agent_in_json(&agents_json, agent_name).is_some(),
                "Should still list {} in agents after using it",
                agent_name
            );
        } else {
            ...
            assert!(
                ...
                "Built-in agent '{}' should not be 'not found': {}",
                agent_name,
                stderr
            );
        }
...
quiet: false"#;
    ...
    assert!(
        list_output.status.success(),
        "Should list agents with hierarchy"
    );
    ...
    let qwen_agent =
        find_agent_in_json(&agents_json, "qwen-coder").expect("Should have qwen-coder agent");
...
async fn test_custom_agent_workflow() -> Result<()> {
    ...
    assert!(
        list_output.status.success(),
        "Should list all agents including custom"
    );

    let agents_json = parse_agent_list_json(&String::from_utf8_lossy(&list_output.stdout))?;

    let user_agent =
        find_agent_in_json(&agents_json, "test-user-agent").expect("Should have custom user agent");
    let project_agent =
        find_agent_in_json(&agents_json, "project-dev").expect("Should have custom project agent");
    ...
    let use_user_output =
        run_sah_command(&["agent", "use", "test-user-agent"], Some(&project_root)).await?;
    ...
    let use_project_output =
        run_sah_command(&["agent", "use", "project-dev"], Some(&project_root)).await?;
...
"#;
    ...
    if use_output.status.success() {
        ...
        assert!(
            ...
            "Should preserve prompt settings"
        );
        assert!(
            updated_config.contains("workflow:"),
            "Should preserve workflow section"
        );
        assert!(
            updated_config.contains("default_timeout"),
            "Should preserve workflow settings"
        );
        assert!(
            updated_config.contains("other_settings:"),
            "Should preserve other settings"
        );
        assert!(
            updated_config.contains("log_level"),
            "Should preserve nested settings"
        );
        assert!(
            updated_config.contains("custom_data:"),
            "Should preserve custom data"
        );
        assert!(
            updated_config.contains("deep_setting"),
        ...
        if switch_output.status.success() {
            ...
            assert!(
                final_config.contains("prompt:"),
                "Should still preserve prompt section"
            );
            assert!(
                final_config.contains("workflow:"),
                "Should still preserve workflow section"
            );
            assert!(
                final_config.contains("other_settings:"),
                "Should still preserve other settings"
            );
            assert!(
                final_config.contains("deep_setting"),
                "Should still preserve deep settings"
            );
...
async fn test_complete_development_workflow() -> Result<()> {
    ...
    let initial_list =
        run_sah_command(&["agent", "list", "--format", "table"], Some(&project_root)).await?;
    assert!(
        initial_list.status.success(),
        "Initial agent list should work"
    );
    ...
    if use_dev.status.success() {
        ...
        if use_qwen.status.success() {
            ...
            let mid_list =
                run_sah_command(&["agent", "list", "--format", "json"], Some(&project_root))
                    .await?;
            assert!(mid_list.status.success(), "Mid-workflow list should work");

            // Step 7: Developer decides to use personal claude setup
            let use_claude =
                run_sah_command(&["agent", "use", "claude-code"], Some(&project_root)).await?;
            ...
            if use_claude.status.success() {
                ...
                let final_list =
                    run_sah_command(&["agent", "list", "--format", "yaml"], Some(&project_root))
                        .await?;
                ...
                assert!(
                    agent_list.len() >= 5,
                    "Should have multiple agents from all sources"
                );
            }
        }
    }
...
async fn test_error_recovery_workflow() -> Result<()> {
    ...
    let bad_use =
        run_sah_command(&["agent", "use", "definitely-not-real"], Some(project_root)).await?;
    assert!(
...
async fn test_concurrent_workflow_safety() -> Result<()> {
    ...
    for (i, agent) in agents.iter().enumerate() {
        ...
        if use_output.status.success() {
            ...
            assert!(
                verify_agent_config(&config_path, agent)?,

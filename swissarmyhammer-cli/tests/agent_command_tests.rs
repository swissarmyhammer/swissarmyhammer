...
async fn run_agent_command_in_dir(args: &[&str], working_dir: &Path) -> Result<std::process::Output> {
    let binary_path = if let Ok(path) = std::env::var("CARGO_BIN_EXE_sah") {
        path
    } else {
        format!(
...
async fn test_agent_list_basic_functionality() -> Result<()> {
    ...
    assert!(stdout.contains("claude-code"), "Should list claude-code agent");
    assert!(stdout.contains("qwen-coder"), "Should list qwen-coder agent");
    assert!(stdout.contains("qwen-coder-flash"), "Should list qwen-coder-flash agent");
    
    // Should show summary information
    assert!(stdout.contains("Agents:"), "Should show agent count summary");
...
async fn test_agent_list_json_format() -> Result<()> {
    ...
    assert!(output.status.success(), "agent list --format json should succeed");
    ...
    assert!(first_agent["name"].is_string(), "Agent should have name field");
    assert!(first_agent["description"].is_string(), "Agent should have description field");
    assert!(first_agent["source"].is_string(), "Agent should have source field");
    
    // Should contain built-in agents
    let agent_names: Vec<_> = agents.iter()
        .map(|agent| agent["name"].as_str().unwrap())
        .collect();
    assert!(agent_names.contains(&"claude-code"), "Should include claude-code");
    assert!(agent_names.contains(&"qwen-coder"), "Should include qwen-coder");
...
async fn test_agent_list_yaml_format() -> Result<()> {
    ...
    assert!(output.status.success(), "agent list --format yaml should succeed");
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    
    // Should be valid YAML
    let yaml_value: serde_yaml::Value = serde_yaml::from_str(&stdout)
...
async fn test_agent_use_builtin_agent() -> Result<()> {
    ...
    if !output.status.success() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("Successfully switched to agent"), "Should show success message");
        assert!(stdout.contains("claude-code"), "Should mention the agent name");
    }
...
async fn test_agent_use_nonexistent_agent() -> Result<()> {
    ...
    assert!(!output.status.success(), "Using nonexistent agent should fail");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("not found"), "Should report agent not found");
    assert!(stderr.contains("nonexistent-agent-xyz"), "Should mention the agent name");
...
async fn test_agent_use_empty_name() -> Result<()> {
    ...
    assert!(!output.status.success(), "Using empty agent name should fail");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("cannot be empty"), "Should report empty name error");
...
async fn test_agent_precedence_user_over_builtin() -> Result<()> {
    ...
    assert!(stdout.contains("custom-test-agent"), "Should list custom user agent");
    assert!(stdout.contains("Custom test agent"), "Should show user agent description");
...
async fn test_agent_precedence_project_over_builtin() -> Result<()> {
    ...
    assert!(output.status.success(), "agent list should succeed in project");
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    
    // Should list project-specific agent
    assert!(stdout.contains("project-dev-agent"), "Should list project agent");
    assert!(stdout.contains("Project-specific development agent"), "Should show project agent description");
    
    // Should show project source counts  
    assert!(stdout.contains("Project:"), "Should show project agent count");
...
async fn test_agent_discovery_hierarchy_full() -> Result<()> {
    ...
    let output = run_agent_command_in_dir(&["agent", "list"], &project_root).await?;
    assert!(output.status.success(), "agent list should succeed with full hierarchy");
    ...
    assert!(stdout.contains("claude-code"), "Should list claude-code (from user or builtin)");
    assert!(stdout.contains("qwen-coder"), "Should list qwen-coder (from project or builtin)");
    assert!(stdout.contains("project-dev-agent"), "Should list project-specific agent");
    assert!(stdout.contains("custom-test-agent"), "Should list user agent");
...
async fn test_agent_use_permission_denied() -> Result<()> {
    ...
    if !output.status.success() {
        ...
        println!("DEBUG: contains permission = {}", stderr_lower.contains("permission"));
        println!("DEBUG: contains configuration = {}", stderr_lower.contains("configuration"));
        println!("DEBUG: contains write = {}", stderr_lower.contains("write"));
        assert!(
            stderr_lower.contains("permission") || stderr_lower.contains("configuration") || stderr_lower.contains("write"),
...
quiet: false"#;
    ...
    assert!(output.status.success(), "agent list should succeed despite invalid files");
    ...
    assert!(!stdout.contains("invalid-agent"), "Should not list invalid agent");
...
async fn test_agent_use_creates_config_file() -> Result<()> {
    ...
    if output.status.success() {
        ...
        assert!(config_content.contains("agent:"), "Config should contain agent section");
        assert!(config_content.contains("executor:"), "Config should contain executor config");
    }
...
"#;
    ...
    if output.status.success() {
        ...
        assert!(updated_config.contains("other_section:"), "Should preserve existing sections");
        assert!(updated_config.contains("value: preserved"), "Should preserve existing values");
        
        // Should add/update agent section
        assert!(updated_config.contains("agent:"), "Should have agent section");
        assert!(updated_config.contains("executor:"), "Should have executor config");
    }
...
async fn test_complete_agent_workflow() -> Result<()> {
    ...
    assert!(list_output.status.success(), "Initial agent list should succeed");
    
    let list_stdout = String::from_utf8_lossy(&list_output.stdout);
    assert!(list_stdout.contains("claude-code"), "Should list claude-code initially");
    
    // Step 2: Use claude-code agent
    let use_output = run_agent_command_in_dir(&["agent", "use", "claude-code"], project_root).await?;
    
    if use_output.status.success() {
        let use_stdout = String::from_utf8_lossy(&use_output.stdout);
        assert!(use_stdout.contains("Successfully switched"), "Should show success");
        ...
        let switch_output = run_agent_command_in_dir(&["agent", "use", "qwen-coder"], project_root).await?;
        
        if switch_output.status.success() {
            let switch_stdout = String::from_utf8_lossy(&switch_output.stdout);
            assert!(switch_stdout.contains("qwen-coder"), "Should show new agent");
            
            // Step 5: Verify config was updated
            let config_content = fs::read_to_string(&config_path)?;
            assert!(config_content.contains("qwen-coder") || config_content.contains("llama-agent"), 
...
async fn test_agent_help() -> Result<()> {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("agent"), "Help should mention agent command");
...
async fn test_agent_use_help() -> Result<()> {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("AGENT_NAME"), "Help should show agent name parameter");

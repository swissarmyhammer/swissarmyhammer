...
async fn test_sah_serve_has_mcp_tools() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(init_response["result"].is_object(), "Initialize should return result object");
    
    // Validate tools capability is advertised
    let capabilities = &init_response["result"]["capabilities"];
    assert!(capabilities["tools"].is_object(), "Server should advertise tools capability");
    ...
    println!("   Memo tools: {}", if has_memo_tools { "✅" } else { "❌" });
    println!("   Issue tools: {}", if has_issue_tools { "✅" } else { "❌" });
    println!("   File tools: {}", if has_file_tools { "✅" } else { "❌" });
    println!("   Notify tools: {}", if has_notify_tools { "✅" } else { "❌" });
    ...
    for (i, tool) in tools.iter().take(5).enumerate() {
        assert!(
            tool["name"].is_string(), 
            "Tool {} should have string name, got: {:?}", i, tool["name"]
        );
        
        // Description should be present (can be null for some tools)
        assert!(
            tool.get("description").is_some(),
            "Tool {} should have description field, got: {:?}", i, tool
        );
        
        assert!(
            tool["inputSchema"].is_object(), 
            "Tool {} should have inputSchema object, got: {:?}", i, tool["inputSchema"]
        );
    }
    ...
    if let Some(notify_tool) = tool_names.iter().find(|&&name| name.contains("notify")) {
        ...
        if let Ok(call_response) = read_response_with_timeout(&mut reader, Duration::from_secs(10)) {
            ...
            if has_result {
                ...
                println!("⚠️ Tool execution returned error (may be expected): {:?}", call_response["error"]);
            }
        }
    }
...
async fn test_sah_binary_exists() {
    ...
    if !output.status.success() {
        println!("❌ sah --help failed with exit code: {:?}", output.status.code());
    ...
    assert!(help_text.contains("serve"), "sah --help should mention 'serve' command");

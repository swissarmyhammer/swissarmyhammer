...
use crate::generation::types::{
    ArgumentConstraint, CliArgument, CliOption, GeneratedCommand, GenerationError, NamingStrategy,
    ParseError,
};
...
impl CommandBuilder {
    ...
    pub fn from_mcp_tool(mut self, tool: &dyn McpTool) -> Result<Self, GenerationError> {
        ...
        let mut command =
            GeneratedCommand::new(command_name, description.to_string(), tool_name.to_string());
    ...
    pub fn build(self) -> Result<GeneratedCommand, GenerationError> {
        self.command
            .ok_or_else(|| GenerationError::CommandBuild("No command initialized".to_string()))
    }
    ...
    fn create_cli_argument(
        ...
        let mut argument = CliArgument::new(cli_name, arg_type, description, is_required);
    ...
    fn create_cli_option(&self, name: &str, schema: &Value) -> Result<CliOption, ParseError> {
        ...
        let mut option = CliOption::new(cli_name, long_name, description, takes_value);
    ...
    fn extract_default_value(&self, schema: &Value) -> Option<String> {
        schema.get("default").map(|v| match v {
        ...
        if let Some(enum_values) = schema.get("enum").and_then(|v| v.as_array()) {
            let values: Result<Vec<String>, _> = enum_values
                ...
                .map(|v| {
                    v.as_str()
                        .ok_or_else(|| {
                            ParseError::TypeConversion("Enum values must be strings".to_string())
                        })
                        .map(|s| s.to_string())
                })
                .collect();
    ...
    fn transform_tool_name(&self, tool_name: &str) -> String {
        match &self.naming_strategy {
            NamingStrategy::KeepOriginal => tool_name.replace('_', "-"),
...
mod tests {
    ...
    fn test_simple_schema_parsing() {
        ...
        let title_arg = command
            .arguments
            .iter()
            .find(|arg| arg.name == "title")
            .unwrap();
    ...
    fn test_constraints_parsing() {
        ...
        assert!(matches!(
            status_arg.constraints[0],
            ArgumentConstraint::Enum(_)
        ));
    }
    ...
    fn test_build_without_initialization() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            GenerationError::CommandBuild(_)
        ));
    }

...
pub fn detect_projects(root: &Path, max_depth: Option<usize>) -> Result<Vec<DetectedProject>, String> {
...
fn check_project_type(path: &Path, project_type: ProjectType) -> Result<Option<DetectedProject>, String> {
...
fn detect_workspace_info(path: &Path, project_type: ProjectType) -> Result<Option<WorkspaceInfo>, String> {
    match project_type {
...
mod tests {
    ...
    fn test_detect_monorepo() {
        ...
        assert!(projects.len() >= 2, "Expected at least 2 projects, found {}", projects.len());
        
        // Check we found both Rust and Node.js
        let has_rust = projects.iter().any(|p| p.project_type == ProjectType::Rust);
        let has_nodejs = projects.iter().any(|p| p.project_type == ProjectType::NodeJs);

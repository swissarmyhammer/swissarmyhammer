...
pub fn render_banner(out: &mut dyn Write, use_color: bool) {
    ...
    if use_color {
        let _ = writeln!(
            out,
            "  {}Standalone MCP shell tool for AI coding agents{}",
            DIM, RESET
        );
    } else {
        let _ = writeln!(out, "  Standalone MCP shell tool for AI coding agents");
    }
    let _ = writeln!(out);
}
...
mod tests {
    ...
    fn banner_plain_contains_shelltool_text() {
        ...
        assert!(output.contains("Standalone MCP shell tool for AI coding agents"));

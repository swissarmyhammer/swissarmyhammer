/**
 * Browser-mode integration test for click-after-scroll focus
 * correctness in `<ColumnView>`. Mounts a column with enough tasks to
 * force virtualization, scrolls, and asserts that clicks on visible
 * cards produce `spatial_focus` IPCs targeting the clicked card's
 * registered key. The snapshot path reads each scope's
 * `getBoundingClientRect()` at click time, so the kernel sees the
 * post-scroll geometry without any continuous rect-update IPC.
 *
 * Test cases:
 *
 * 1. `click_card_at_top_of_scrolled_column_focuses_it` — scroll the
 *    column and click a card whose viewport-y has shifted ≥ 200 px
 *    from its mount-time position.
 * 2. `click_each_visible_card_after_scroll_focuses_each` — scroll, then
 *    iterate every visible card and click each.
 * 3. `click_card_immediately_after_scroll_into_view_focuses_it` —
 *    scrolls programmatically and clicks the now-visible card without
 *    waiting beyond the same animation frame.
 * 4. `non_scrolling_column_click_still_focuses_card` — a column with
 *    too few tasks to scroll keeps working as today.
 */
 ...
 * ancestor (inline `overflow-y: auto`).
 *
                    ...
                    <ActiveBoardPathProvider value="/test/board">
                      <div
                        ...
                        style={{
                          ...
                          // (every card visible, no scrolling). Drive
                          // scroll from the OUTER wrapper instead: make
                          // this div the scrollable ancestor with
...
describe("<ColumnView> — click reliability across scroll positions", () => {
  beforeEach(() => {
  ...
  // -------------------------------------------------------------------------
  // 1. Click on a card after scroll focuses it.
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // 2. Click each visible card after scroll — focus tracks every click.
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // 3. Click immediately after scroll — no extra delay between scroll and
  ...
  // -------------------------------------------------------------------------
  // 4. Non-regression: a column with too few tasks to scroll keeps working.
  // -------------------------------------------------------------------------

...
const TASK_SCHEMA = {
  entity: {
    name: "task",
    body_field: "body",
    fields: ["title", "tags", "body", "assignees"],
  },
  fields: [
    {
      id: "f1",
...
const ACTORS: Entity[] = [
  {
    entity_type: "actor",
    id: "alice-id",
    fields: { name: "Alice Smith", color: "3366cc" },
  },
  {
    entity_type: "actor",
    id: "bob-id",
    fields: { name: "Bob Jones", color: "cc3366" },
  },
];
...
describe("CellDispatch", () => {
  describe("avatar display (assignees)", () => {
    ...
    it("avatars overlap with negative margin", () => {
      ...
      const hasOverlap = Array.from(avatars).some((a: Element) =>
        a.className.includes("-ml-1.5"),
      );
    ...
    it("single assignee has no overlap class", () => {
      const { container } = renderCell(ASSIGNEES_FIELD, ["alice-id"], {
        actor: ACTORS,
      });
  ...
  describe("badge-list display (tags)", () => {
    it("renders MentionPill components for tag IDs", async () => {
    ...
    it("empty tags shows dash", async () => {
  ...
  describe("grid vs inspector parity", () => {
    ...
    it("assignees render identically in grid and inspector", async () => {
      ...
      const grid = renderCell(ASSIGNEES_FIELD, ["alice-id", "bob-id"], {
        actor: ACTORS,
      });
      ...
      const inspectorRow = inspector.container.querySelector(
        '[data-testid="field-row-assignees"]',
      );

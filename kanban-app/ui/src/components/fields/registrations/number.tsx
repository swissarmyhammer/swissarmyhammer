...
function NumberEditorAdapter({ value, onCommit, onCancel }: FieldEditorProps) {
  return <NumberEditor value={value} onCommit={onCommit} onCancel={onCancel} mode="compact" />;
}

/** Number display adapter — wraps NumberDisplay to match FieldDisplayProps. */
function NumberDisplayAdapter({ field, value, entity, mode }: FieldDisplayProps) {
  return <NumberDisplay field={field} value={value} entity={entity!} mode={mode} />;
}

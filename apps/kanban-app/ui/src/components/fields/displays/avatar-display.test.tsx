...
describe("AvatarDisplay", () => {
  it("renders dash for empty array when no placeholder is set", async () => {
    await renderDisplay([]);
    expect(screen.getByText("-")).toBeTruthy();
  });

  it("renders dash for null/undefined when no placeholder is set", async () => {
    await renderDisplay(null);
    expect(screen.getByText("-")).toBeTruthy();
  });

  it("renders Avatar components for array of actor IDs", async () => {
    const actors = [makeActor("alice", "Alice Smith")];
    await renderDisplay(["alice"], actors);
    // Should render initials for Alice
    expect(screen.getByText("AS")).toBeTruthy();
  });

  it("renders an image directly for a string URL value", async () => {
    const { container } = await renderDisplay(DATA_URI);
    const img = container.querySelector("img");
    expect(img).not.toBeNull();
    expect(img!.src).toBe(DATA_URI);
  });

  it("renders an image for an https URL string", async () => {
    const url = "https://example.com/avatar.png";
    const { container } = await renderDisplay(url);
    const img = container.querySelector("img");
    expect(img).not.toBeNull();
    expect(img!.src).toBe(url);
  });

  it("renders dash for empty string when no placeholder is set", async () => {
    await renderDisplay("");
    expect(screen.getByText("-")).toBeTruthy();
  });

  it("image is rendered as a circle (rounded-full)", async () => {
    const { container } = await renderDisplay(DATA_URI);
    const img = container.querySelector("img")!;
  ...
  it("renders the configured placeholder in compact mode when value is empty", async () => {
  ...
  it("renders the configured placeholder in full mode with italic styling when value is empty", async () => {
    const fieldWithPlaceholder: FieldDef = {
      ...DEFAULT_FIELD,
      placeholder: "Assign",
    } as unknown as FieldDef;
  ...
  it("renders italic 'None' fallback in full mode when no placeholder is set", async () => {
    const { container } = await renderDisplay([], [], { mode: "full" });
    const hint = container.querySelector("span.italic");
    expect(hint).toBeTruthy();
    expect(hint?.textContent).toBe("None");
  });

  it("falls back to dash in compact mode when no placeholder is set (regression guard)", async () => {
  ...
  it("compact-mode populated and empty variants share the CompactCellWrapper contract", async () => {
    const actors = [makeActor("alice", "Alice Smith")];

    const populated = await renderDisplay(["alice"], actors, {
      mode: "compact",
    });
  ...
  it("full-mode does not wrap output in the compact-cell wrapper", async () => {
    const actors = [makeActor("alice", "Alice Smith")];
    const { container } = await renderDisplay(["alice"], actors, {
      mode: "full",
    });
    expect(container.querySelector("[data-compact-cell='true']")).toBeNull();
  });

  it("renders Avatar at sm size in compact mode so it fits in the normalized row", async () => {
    const actors = [makeActor("alice", "Alice Smith")];
    const { container } = await renderDisplay(["alice"], actors, {
      mode: "compact",
    });

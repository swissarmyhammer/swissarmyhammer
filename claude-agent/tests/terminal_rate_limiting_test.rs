...
#[tokio::test]
async fn test_terminal_capability_enforcement() {
    ...
    // Test 1: Operations should fail when no client capabilities are set
    ...
    let result = manager
        .create_terminal_with_command(&session_manager, params.clone())
        .await;
    assert!(
        result.is_err(),
        "Terminal creation should fail without client capabilities"
    );
    match result {
        Err(claude_agent::AgentError::Protocol(msg)) => {
            assert!(
                msg.contains("capabilities") || msg.contains("capability"),
                "Error should mention capabilities, got: {}",
                msg
            );
        }
        Err(other) => panic!(
    ...
    // Test 2: Operations should fail when terminal capability is explicitly disabled
    manager
    ...
    assert!(
        result.is_err(),
        "Terminal creation should fail when terminal capability is disabled"
    );
    match result {
        Err(claude_agent::AgentError::Protocol(msg)) => {
            assert!(
    ...
    // Test 3: Operations should succeed when terminal capability is enabled
    manager
    ...
    assert!(
        result.is_ok(),
        "Terminal creation should succeed with terminal capability enabled, got: {:?}",
        result.err()
    );
}
...
#[tokio::test]
async fn test_capability_enforcement_on_get_output() {
    let rate_limiter = RateLimiter::with_config(RateLimiterConfig {
        per_client_limit: 100,
        global_limit: 1000,
        expensive_operation_limit: 500,
    ...
    // Attempt to get output - should fail due to missing capability
    let result = manager.get_output(&session_manager, params).await;
    assert!(
        result.is_err(),
        "get_output should fail without terminal capability"
    );
    match result {
        Err(claude_agent::AgentError::Protocol(msg)) => {
            assert!(
...
#[tokio::test]
async fn test_capability_enforcement_on_kill() {
    let rate_limiter = RateLimiter::with_config(RateLimiterConfig {
        per_client_limit: 100,
        global_limit: 1000,
        expensive_operation_limit: 500,
    ...
    // Attempt to kill terminal - should fail due to missing capability
    let result = manager.kill_terminal(&session_manager, params).await;
    assert!(
        result.is_err(),
        "kill_terminal should fail without terminal capability"
    );
    match result {
        Err(claude_agent::AgentError::Protocol(msg)) => {
            assert!(
...
#[tokio::test]
async fn test_capability_enforcement_on_release() {
    let rate_limiter = RateLimiter::with_config(RateLimiterConfig {
        per_client_limit: 100,
        global_limit: 1000,
        expensive_operation_limit: 500,
    ...
    // Attempt to release terminal - should fail due to missing capability
    let result = manager.release_terminal(&session_manager, params).await;
    assert!(
        result.is_err(),
        "release_terminal should fail without terminal capability"
    );
    match result {
        Err(claude_agent::AgentError::Protocol(msg)) => {
            assert!(

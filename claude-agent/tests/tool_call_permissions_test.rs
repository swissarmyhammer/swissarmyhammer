...
async fn test_fs_write_fails_without_write_capability() {
    ...
    match result {
        ...
        Ok(other) => {
            panic!(
                "fs_write should fail when write_text_file capability is false, got: {:?}",
                other
            );
        }
    }
...
async fn test_terminal_fails_without_terminal_capability() {
    ...
    match result {
        ...
        Ok(other) => {
            panic!(
                "terminal_create should fail when terminal capability is false, got: {:?}",
                other
            );
        }
    }
...
async fn test_operations_fail_with_no_capabilities() {
    ...
    match write_result {
        ...
        Ok(other) => panic!(
            "fs_write should fail or require permission with no capabilities, got: {:?}",
            other
        ),
    }
    ...
    match terminal_result {
        ...
        Ok(other) => panic!(
            "terminal_create should fail with no capabilities, got: {:?}",
            other
        ),
    }
}

...
mod tests {
    ...
    async fn test_full_prompt_flow() {
        ...
        match &session.context[0].update {
        ...
        match &session.context[1].update {
    ...
    async fn test_conversation_context_maintained() {
        ...
        // Verify session has messages from both turns
        let session_id = new_session_response.session_id.0.as_ref().parse().unwrap();
        let session = agent
            .session_manager
            .get_session(&session_id)
        ...
        // Should have at least 2 user messages + their responses
        assert!(session.context.len() >= 4);

        // Find user messages
        let user_messages: Vec<String> = session
            .context
            .iter()
        ...
        assert_eq!(user_messages[0], "My name is Alice");
        assert_eq!(user_messages[1], "What is my name?");
    ...
    async fn test_streaming_prompt() {
        ...
        // Verify session was updated with user message and assistant chunks
        let session = agent
            .session_manager
            .get_session(&session_id)
            .unwrap()
        ...
        // Should have user message + multiple assistant chunk messages
        assert!(
            session.context.len() >= 2,
            "Expected at least user + assistant chunks"
        );

        // First message should be UserMessageChunk
        assert!(
            matches!(
        ...
        // Remaining messages should be AgentMessageChunk
        for msg in &session.context[1..] {
            assert!(
                matches!(msg.update, SessionUpdate::AgentMessageChunk(..)),
                "Assistant messages should be AgentMessageChunk"
            );
        }
    }
    ...
    async fn test_streaming_session_context_maintained() {
        ...
        // Verify session has messages from both turns (user messages + assistant chunks)
        let session = agent
            .session_manager
            .get_session(&session_id)
            .unwrap()
        ...
        // Should have multiple messages: user1 + assistant chunks + user2 + assistant chunks
        assert!(
            session.context.len() >= 4,
            "Expected messages from both conversation turns"
        );

        // Find user messages to verify conversation structure
        let user_messages: Vec<String> = session
            .context
            .iter()
            .filter_map(|m| match &m.update {
        ...
        let assistant_message_count = session
            .context
            .iter()
            .filter(|m| matches!(m.update, SessionUpdate::AgentMessageChunk(..)))
            .count();

        assert!(
            assistant_message_count >= 2,
            "Should have assistant chunk messages"
        );
    ...
    async fn test_streaming_prompt_with_resource_link() {
        ...
        // First message should be UserMessageChunk with prompt text and resource link
        match &session.context[0].update {
            SessionUpdate::UserMessageChunk(chunk) => {
                if let ContentBlock::Text(text) = &chunk.content {
                    assert!(
                        text.text
                            .contains("I'm providing a resource link for reference"),
                        "User message should contain the prompt text"
                    );
                    assert!(
                        text.text.contains("https://example.com/document.pdf"),
                        "User message should contain the resource link URL"
                    );
                } else {
                    panic!("Expected Text content");
                }
    ...
    async fn test_turn_counter_resets_between_prompts() {
        ...
        // according to ACP specification. We test this by calling prompt multiple times through
        // the ACP protocol and verifying each completes successfully (would fail if counter accumulated).

        let local_set = tokio::task::LocalSet::new();
        local_set
            .run_until(async {
                // Use proper ACP connection for integration testing
                ...
                let response_1 = agent.prompt(prompt_request_1).await.unwrap();
                assert_eq!(
                    response_1.stop_reason,
                    StopReason::EndTurn,
                    "First prompt should complete normally"
                );

                // Second prompt - should RESET counter and complete successfully
                // If counter accumulated, this would fail due to max_turn_requests limit
                let prompt_request_2 = PromptRequest {
                    session_id: session_response.session_id.clone(),
                    prompt: vec![ContentBlock::Text(TextContent {
                ...
                let response_2 = agent.prompt(prompt_request_2).await.unwrap();
                assert_eq!(
                    response_2.stop_reason,
                    StopReason::EndTurn,
                    "Second prompt should complete normally - proves counter was reset"
                );
            })
            .await
    }

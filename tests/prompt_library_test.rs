...
fn test_comprehensive_prompt_library() {
    ...
    let mut prompt_list: Vec<_> = loader.storage.iter().map(|(name, _)| name).collect();
    prompt_list.sort();
    ...
    assert!(
        prompt_count >= 15,
        "Expected at least 15 prompts, found {}",
        prompt_count
    );
    ...
    for expected in &expected_prompts {
        let found = loader
            .storage
            .iter()
            .any(|(name, _)| &name == expected || name.ends_with(expected));
    ...
    for (name, prompt) in loader.storage.iter() {
        ...
        assert!(
            ...
            "Prompt '{}' has empty content",
            name
        );
        assert!(
            !prompt.source_path.is_empty(),
            "Prompt '{}' has empty source_path",
            name
        );

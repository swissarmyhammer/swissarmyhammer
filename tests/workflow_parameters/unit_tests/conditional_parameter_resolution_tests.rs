...
mod basic_conditional_parameter_tests {
    use super::*;
    use swissarmyhammer::common::parameter_conditions::ParameterCondition;
    ...
    fn test_multiple_conditions_or_logic() {
        let params = vec![
            Parameter::new("env", "Environment", ParameterType::String)
                .required(true),
            Parameter::new("urgent", "Urgent", ParameterType::Boolean)
                .with_default(json!(false)),
    ...
    fn test_complex_branching_conditions() {
        let params = vec![
            Parameter::new("service_type", "Service type", ParameterType::Choice)
                .with_choices(vec!["web".to_string(), "api".to_string(), "worker".to_string()])
                .required(true),
        ...
        let result = resolve_params(
            &params,
            &[("service_type", "worker"), ("worker_queue", "high_priority")],
            false,
        ).unwrap();
        assert_eq!(result.len(), 2);
    ...
    fn test_cross_parameter_dependencies() {
        let params = vec![
            Parameter::new("auth_provider", "Auth provider", ParameterType::Choice)
                .with_choices(vec!["local".to_string(), "oauth".to_string(), "saml".to_string()])
                .required(true),
            Parameter::new("oauth_client_id", "OAuth Client ID", ParameterType::String)
                .when("auth_provider == 'oauth'")
                .required(true),
            Parameter::new("oauth_client_secret", "OAuth Client Secret", ParameterType::String)
                .when("auth_provider == 'oauth'")
                .required(true),
        ...
        assert_eq!(result.get("oauth_client_secret").unwrap(), &json!("secret456"));
        ...
        assert_eq!(result.get("saml_cert_path").unwrap(), &json!("/etc/saml/cert.pem"));
...
mod conditional_parameter_edge_cases {
    ...
    fn test_circular_dependency_detection() {
        // This should be caught by the MAX_ITERATIONS limit
        let params = vec![
        ...
        let result = resolve_params(&params, &[], false);
        assert!(result.is_err());
        match result.unwrap_err() {
            ParameterError::ValidationFailed { message } => {
                assert!(message.contains("circular dependency"));
            }
            _ => panic!("Expected ValidationFailed error for circular dependency"),
        }
    }
    ...
    fn test_iterative_resolution_with_multiple_dependencies() {
        ...
        assert_eq!(result.get("derived_param1").unwrap(), &json!("derived1_value"));
        assert_eq!(result.get("derived_param2").unwrap(), &json!("derived2_value"));
    ...
    fn test_conditional_parameter_overriding_defaults() {
        ...
        assert_eq!(result.get("advanced_config").unwrap(), &json!("default_advanced_config"));
        ...
        assert_eq!(result.len(), 2);
        assert_eq!(result.get("mode").unwrap(), &json!("advanced"));
        assert_eq!(result.get("advanced_config").unwrap(), &json!("custom_config"));
    }
}
...
mod conditional_parameter_error_handling {
    ...
    fn test_conditional_parameter_missing_error_details() {
        let params = vec![
            Parameter::new("database_enabled", "Enable database", ParameterType::Boolean)
                .required(true),
    ...
    fn test_regular_vs_conditional_parameter_missing_errors() {
        let params = vec![
            Parameter::new("required_param", "Required", ParameterType::String)
                .required(true),
            Parameter::new("conditional_param", "Conditional", ParameterType::String)
                .when("trigger == true")
                .required(true),
        ...
        let result = resolve_params(&params, &[("required_param", "value"), ("trigger", "true")], false);
        assert!(result.is_err());
        match result.unwrap_err() {
            ParameterError::ConditionalParameterMissing { parameter, condition } => {

...
| `01KS36RT7F7WZMNBCHER0HRGKM` | Builtin plugin: column/attachment/tag/view (4 small YAMLs) | sdk-helpers, catalog, window, views | `kanban-misc-commands` (5): column/tag → kanban, attachment open/reveal → window, view.set → views. |
| `01KS36SEXMBGZJTWJX0ZQQKP8V` | Builtin plugin: file/board commands (port file.yaml) | sdk-helpers, catalog, window | `file-commands` (4): switch/close/new/openBoard → window board lifecycle. |
...
| `01KS36TCNMSDGSQBZP3NKY6YK7` | Builtin plugin: entity + clipboard (entity.yaml) | sdk-helpers, catalog, kanban-ext | `entity-commands` (8): CRUD + archive + clipboard → extended kanban; `from: target`. |
| `01KS36TSWE3NR5MFQTY99JX5TB` | Builtin plugin: ui commands (ui.yaml, 10 incl. window.new) | sdk-helpers, catalog, ui_state, window | `ui-commands` (10) → ui_state, window.new → window; `ui.inspect` via backend (no client shortcut). |
| `01KS36V80DXK2BFDDSHSWP131W` | Builtin plugin: app + settings + drag (app/settings/drag.yaml) | sdk-helpers, catalog, app, ui_state, store | `app-shell-commands` (15): quit/about/help → app, undo/redo → store, command/palette/search/dismiss + keymap + drag → ui_state. |

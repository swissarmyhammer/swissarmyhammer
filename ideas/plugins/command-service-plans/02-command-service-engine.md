...
| `01KS613VPH2G4ZWKZPGW9ZCJAA` | execute transaction bracketing + `commands/executed` action event | execute/available (`01KS36P9C8`), store (`01KS5F7BR6`), notif surface (`01KS5G3AKZ`) | wraps execute: opens/closes a `txn` (one undo group), emits `commands/executed {id,ctx,result,txn,origin}` correlated to the `store/changed` events; write-nothing commands still emit; errors still close the txn. |

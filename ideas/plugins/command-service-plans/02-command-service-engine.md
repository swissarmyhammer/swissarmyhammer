...
| `01KS36P9C8CFT5HMQWY2WCA9ZE` | execute + available verbs (callback round-trip + latency budget + txn-bracket + action emit) | list/schema | execute rechecks available (unless force), brackets a `txn`, emits `commands/executed`; available enforces 5ms/50ms budget. |
...
- **`execute` is the command-as-unit boundary**: opens a `txn`, runs the
  ...
  empty (free) undo group and still emit the action event.
- SDK convention: a plugin's `load()` does `ensureServices(...)` then
  `registerCommands(...)`. Relies on plugin-arch idempotent registration.

## Cross-check

`kanban list tasks --filter '$command-service'` → expect exactly these 7 tasks.

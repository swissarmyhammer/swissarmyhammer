# Skip .sah Files in Rule Checker
...
Rule checker needs to skip files in `.sah/` directory, but should do this intelligently using the SwissarmyhammerDirectory from context, not by doing string matching.
...
**Current reliance**: .gitignore to exclude .sah files

## Issues with Current Approach

### 1. Depends on .gitignore Configuration
- Only works if .sah is in .gitignore
...
- Can't distinguish between:
  - `.sah/` (should skip)
  - `node_modules/.sah/` (maybe should skip?)
  - `vendor/.sah/` (maybe should skip?)

### 3. String-Based Would Be Fragile
```rust
// WRONG approach (dumb string matching):
target_files.retain(|path| {
    !path.to_string_lossy().contains(".sah")
});
```

This would incorrectly skip:
- `/projects/.sah-backup/src/main.rs`
- `/code/my-.sah-test/lib.rs`
- Any path with ".sah" substring

### 4. No Flexibility
- Can't check specific files in .sah if needed
...
impl SwissarmyhammerDirectory {
    /// Check if a path is within the .sah directory
...
### 4. Explicit
- Clear intent: "skip the .sah directory"
...
# Line 63-67: Some test-specific .sah entries
swissarmyhammer/.sah
swissarmyhammer-agent-executor/.sah
swissarmyhammer-cli/.sah
swissarmyhammer-workflow/.sah
swissarmyhammer-tools/.sah
```

But NOT a global `.sah/` entry at the root!

So currently, .sah files at the project root ARE being checked by rules.
...
4. Add tests verifying .sah files are skipped

...
**THIS IS A SCRATCH FILE GENERATED BY A 🤖**
...
- ✅ **Primary caller**: AI agents via MCP, not users directly
- ✅ **No validation**: Just write the file, don't validate
- ✅ **Directory creation**: Create directories if they don't exist (mkdir -p behavior)
...
### Replacement Pattern
- ✅ "next item to work on" → `todo_show` with `item="next"`
- ✅ "are we done?" → Check if todo.yaml exists / all todos complete
- ✅ "mark complete" → `todo_mark_complete`
- ✅ "find TODOs in code" → Create "no-todos" rule, violations auto-create todos
- ✅ "implementation notes" → Code comments (user decision)
- ✅ "proposed solution" → Just implement (or put in scratch file if needed)

### Workflows to Delete
- `do_issue.md` → Replace with `do_todos` (already exists!)
- `implement.md` → Replace with simpler "do all todos" workflow
...
- `do_issue` → DELETE
- `implement` → DELETE
- `do_todos` → RENAME to `do` (becomes the main implementation loop)
...
### New behavior: ✅ OPTION B - Rules + Todos
...
4. As implementation progresses:
   ...
   - Complete todos → auto-GC after 24h

### What we keep
- ✅ Rich context (markdown, diagrams, examples in todo context field)
- ✅ Permanent success criteria (rules)
- ✅ Implementation steps (todos)

### What we lose
- ❌ Numbered individual markdown files per step
- ❌ Ability to "update" a todo during implementation (but notes go in code comments anyway)
- ❌ Permanent record of implementation steps (but rules remain as the spec)
...
## System Prompts
- ✅ ANSWER: Just delete memo_get_all_context usage - no replacement needed
...
- `todo_list` MCP tool (inspired by `issue_list` - list all todos) ✅ DONE
- `sah do` CLI command (runs the `do` workflow)
- `sah todos` CLI command (lists all current todos)

### What gets renamed
- `do_todos` workflow → `do` workflow
...
1. ✅ **rule_create design** - COMPLETE
2. ✅ **Workflow architecture** - COMPLETE
3. ✅ **Plan workflow update** - COMPLETE (creates rules + rich todos)
...
1. ✅ **Builtin rules format**: Keep as-is (don't migrate), only use minimal format for new project-specific rules
2. ✅ **Spec file after plan**: Leave it where it is (user manages it)
3. ✅ **rule_create directory handling**: Create directories if they don't exist (mkdir -p behavior)
4. ✅ **CLI command name**: `sah do` (simple, matches workflow name)
5. ✅ **Delete issues/memos in git**: Don't touch them, just ignore them (add to .gitignore)
6. ✅ **Todo list command**: YES - need `todo_list` MCP tool and `sah todos` CLI command (✅ MCP tool done)

...
| `assign task` | Assign task to an actor | `op: "assign task", id: "<task_id>", assignee: "<actor_id>"` |
...
2. **Create** - Add each task to the board with `add task`
3. **Assign** - Assign tasks to yourself with `assign task`
4. **Execute** - Work through tasks one at a time
5. **Complete** - Mark each task done immediately after finishing
...
```
kanban op: "add comment", task_id: "<task_id>", body: "Found root cause - null pointer in auth module", author: "claude"
```

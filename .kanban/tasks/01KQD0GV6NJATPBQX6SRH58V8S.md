...
> **Closed 2026-04-29 (claude-code)**: consumers task landed; migrated the last root-level schema import in `avp_schema_tests.rs` (`ContentChunk`, `SessionId`, `SessionNotification`, `SessionUpdate` → `agent_client_protocol::schema::*`). `cargo check -p agent-client-protocol-extras --tests` clean. `cargo nextest run -p agent-client-protocol-extras --test e2e_hooks` reports 67/67 tests passing (all 18 `avp_schema_tests` included). Committed as `96e368b4f` on `acp/0.11-rewrite`.

## Acceptance Criteria
- [x] `cargo check -p agent-client-protocol-extras --tests` passes.
- [x] `mod avp_schema_tests;` is **enabled** in `tests/e2e_hooks/main.rs` (no comment-out, no `#[cfg]` gate).
- [x] `avp-common` is in `agent-client-protocol-extras/Cargo.toml` `[dev-dependencies]`.
- [x] All schema-type imports inside `tests/e2e_hooks/*.rs` use `agent_client_protocol::schema::*` paths. (Outstanding: `avp_schema_tests.rs:200`.)
- [x] One commit on `acp/0.11-rewrite`.

## Tests
- [x] `cargo nextest run -p agent-client-protocol-extras --test e2e_hooks` — all 50+ tests green, including `avp_schema_tests`.

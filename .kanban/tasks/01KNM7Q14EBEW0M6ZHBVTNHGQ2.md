...
## Acceptance Criteria
- [ ] Computed fields with `depends_on: [\"_changelog\"]` receive changelog data in `fields[\"_changelog\"]`
- [ ] Computed fields without `_changelog` dependency are unaffected
- [ ] `_changelog` key is stripped from entity fields after derivation
- [ ] Entities with no changelog (new/empty) get `_changelog: []`
- [ ] No changelog read when no computed fields need it

## Tests
- [ ] Unit test: entity with changelog-dependent computed field → verify `_changelog` available during derivation
- [ ] Unit test: entity with only non-changelog computed fields → verify no changelog read (no `_changelog` key injected)
- [ ] Unit test: `_changelog` is stripped after derivation (not visible in returned entity)

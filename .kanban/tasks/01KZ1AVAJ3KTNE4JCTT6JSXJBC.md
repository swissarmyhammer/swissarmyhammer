...
- actor: claude-code
  id: 01kz21j8s4w72yawab6zdgfyx3
  text: |-
    ### finish iteration 1 — changed
    - implement: changed — real root cause was sharper than the card's hypothesis: a 14.9 MB prompt against a live 5 MB cap (swissarmyhammer-agent already overrode the 100KB claude-agent default). Dominant cost was the duplicates probe's <changed-set> evidence (~1.43MB) repeated once per file, not rendering overhead — filed separately as ^t7f5fqf since it's a waste defect, not this card's cap-mismatch defect. Fix: MAX_PROMPT_LENGTH is now declared once (claude-agent/src/constants/sizes.rs, 512KiB) and read everywhere; batch_work_list takes a cost closure and budgets rendered bytes with no fixed multiplier; validate_prompt_request names the actual length and limit instead of bare invalid_params; AgentPool::enqueue refuses an over-cap prompt before sending (PoolError::PromptTooLong) and SkippedFile reports the (validator,file) pairs that couldn't be packed.
    - test: green — independently verified: cargo nextest run -p claude-agent -p swissarmyhammer-agent -p swissarmyhammer-validators -p swissarmyhammer-tools -p acp-conformance => 3318/3318 passed. fmt/clippy clean.
    - note: `review sha 0c8b969b8~1..0c8b969b8` (the real 71-file reproduction) was NOT run live — substituted with a new production-path test (`every_prompt_a_packed_batch_sends_fits_inside_the_agent_prompt_cap`) that packs a comparably large multi-file batch through the real batching/rendering pipeline.
    - also in this commit, user-directed and confirmed: deletion of two real-model e2e tests (kanban-app ai_panel_e2e, swissarmyhammer-agent review_real_model_e2e) stalling under GPU contention from an unrelated parallel worktree run, plus the now-dangling nextest.toml overrides.
    - commit: 0ecaff64a (fix), 1feafefb0 (follow-up card filing)
    - next: /review HEAD~1..HEAD — this is the first real test of the fix, reviewing the fix itself through the now-corrected engine
  timestamp: 2026-08-02T20:10:09.572457+00:00
- actor: claude-code
  id: 01kz22cwvt5sq9k5nz347kz8nt
  text: '### review — stuck\n\n`review sha 0ecaff64a~1..0ecaff64a` (the commit under review here) does NOT run. It hard-errors before any fleet task is attempted — this is not the `invalid_params` symptom this card fixed, it is a different top-level pipeline error:\n\n```\nMCP error -32603: review pipeline failed: Validator ''complexity'' error: unknown probe ''complexity''; the catalog defines: callers, duplicates, similar\n```\n\nEvidence gathered before reporting, so this is not a guess:\n\n- The error is a hard error, not a `counts.failed` tally — `attempted`/`failed`/`skipped` never populate because the pipeline aborts during validator/probe resolution, before batching.\n- `crates/swissarmyhammer-validators/src/review/probes.rs` at current HEAD (`1feafefb0`) DOES define a `complexity` entry in `CATALOG` (added in `8d7d8f57dd`, an ancestor of `0ecaff64a`), so the source is not missing it. The running engine''s resolved catalog is reporting only 3 entries (`callers, duplicates, similar`) — the catalog the live process is using disagrees with the catalog in the source tree at this commit.\n- Reproducible: `review sha 0ecaff64a~1..0ecaff64a` fails every time (3 attempts). `review sha 8d7d8f57dd~1..8d7d8f57dd` (the commit that added the `complexity` probe) fails identically. `review sha HEAD~1..HEAD` (the kanban-only follow-up commit, no `.rs` files) succeeds with an empty scope. So the trigger is: any diff scope containing `.rs` files pulls in the `complexity` validator by glob match, and probe resolution for it fails.\n- Passing `validators: [\"rust\"]` or `validators: [\"command-safety\", ...]` (excluding `\"complexity\"` by name) does NOT avoid the error — the `complexity` validator still gets matched and probe-resolved regardless of the `validators` passthrough filter. This looks like a second, separate defect in validator selection.\n- `review file <single-path>` (whole-file mode, not diff mode) on `crates/claude-agent/src/agent.rs` succeeds normally with findings. So the failure is specific to `review sha` (diff-scoped) resolution, not global to the engine.\n\nConclusion: I cannot verify this card''s fix (`0ecaff64a`) against a real `review sha` run today — the review engine itself is blocked by an unrelated-looking but currently-real defect (probe catalog / validator-selection mismatch for `complexity`, introduced by `8d7d8f57dd`, a card `^k5wsxh0` change). Per instruction, I am NOT treating this as a clean pass and NOT moving this task. Leaving it in `doing` for a human to decide: either fix the `complexity` probe registration gap (or the `validators` passthrough bug) first, or find another way to exercise the batch-budget fix through `review sha`.\n\nnext: re-run `/review 01KZ1AVAJ3KTNE4JCTT6JSXJBC 0ecaff64a~1..0ecaff64a` once the complexity-probe registration issue is fixed.'
  timestamp: 2026-08-02T20:24:42.106580+00:00
- actor: claude-code
  id: 01kz2j241dsnrd3255ynje9ssd
  text: |-
    ### review — skipped (user override)
    Review could not run: `review sha` hard-errors on any `.rs`-touching diff with `unknown probe 'complexity'` (a separate defect introduced by ^k5wsxh0, not by this card). Confirmed against a freshly reconnected server matching current HEAD (1feafefb0) — not a stale-binary artifact. Moving this card forward requires fixing the probe-catalog bug first, which is a circular dependency: the tool needed to verify this fix is broken by a different fix in the same batch.

    User explicitly instructed skipping review for this card and ^k5wsxh0. Moved to `done` on that basis, not on a clean review result. The implementation evidence stands on its own: independently verified `cargo nextest run -p claude-agent -p swissarmyhammer-agent -p swissarmyhammer-validators -p swissarmyhammer-tools -p acp-conformance` => 3318/3318 passed, fmt/clippy clean, and the core claims (single MAX_PROMPT_LENGTH source, diagnostic rejection message, PoolError::PromptTooLong wired into AgentPool::enqueue) were spot-checked directly in source, not just trusted from the implementer's report.

    Follow-up: the complexity probe-catalog bug found while trying to review this card needs its own card so it doesn't get lost.
  timestamp: 2026-08-03T00:58:26.221780+00:00

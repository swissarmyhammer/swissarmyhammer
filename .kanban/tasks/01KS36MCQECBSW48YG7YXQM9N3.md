...
## Acceptance Criteria
- [x] `CommandService` registers as an `rmcp::ServerHandler` and `tools/list` returns one tool named `command` with full `_meta` operations tree
- [x] `CommandRegistry::push` enforces the per-caller dedupe rule (B pushes `foo` twice → still one B entry, on top)
- [x] `CommandRegistry::pop_caller` followed by `active` returns the previous entry (the stack-fallback rule)
- [x] `purge_caller` removes every entry from one caller in a single call
- [x] Notifications are debounced to ~100ms and a `flush()` call drains pending notifications immediately

## Tests
- [x] `crates/swissarmyhammer-command-service/tests/registry_stack.rs` — exercises push/pop_caller/active/purge directly. Includes the architecture-doc scenario: host registers `core.archive`; plugin A overrides; plugin B overrides; B unloads → A active; A unloads → host active.
- [x] `crates/swissarmyhammer-command-service/tests/registry_dedupe.rs` — same caller registering the same id twice → one entry on the stack, not two
- [x] `crates/swissarmyhammer-command-service/tests/notifications_debounce.rs` — 5 rapid register calls in one caller produce one notification; a `flush()` boundary forces immediate emission
- [x] `crates/swissarmyhammer-command-service/tests/server_handler_smoke.rs` — `tools/list` on the rmcp service returns the `command` tool and its `_meta` operations tree has all six verbs under noun `command`
...
- Added `Hash` derive to `CallerId` in `swissarmyhammer-plugin/src/server.rs` so it can be used in the registry's per-caller dedupe scan (PluginId already had Hash).
- Notification debounce uses tokio's real clock (40ms window in tests) — avoids depending on tokio's `test-util` feature for the `start_paused`/`advance` test helpers.
- Verb handlers route through a single `call_tool` dispatcher; the per-verb bodies are `todo!()` stubs for follow-up tasks.
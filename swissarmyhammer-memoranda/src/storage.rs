...
impl MarkdownMemoStorage {
    ...
    pub async fn new_default() -> Result<Self> {
        let memos_dir = swissarmyhammer_common::utils::paths::get_swissarmyhammer_dir()
            .map_err(|e| MemorandaError::Storage(format!("Failed to get SwissArmyHammer directory: {}", e)))?
            .join("memos");
    ...
    async fn load_memo_from_file(&self, file_path: &PathBuf, title: &MemoTitle) -> Result<Memo> {
        ...
        let updated_at = metadata.modified()
            .map_err(|e| MemorandaError::Storage(format!("Failed to get modification time: {}", e)))?
            .into();
...
impl MemoStorage for MarkdownMemoStorage {
    async fn create(&mut self, title: MemoTitle, content: MemoContent) -> Result<Memo> {
        ...
        if file_path.exists() {
            return Err(MemorandaError::InvalidOperation(format!(
                "Memo with title '{}' already exists", title
            )));
        }
    ...
    async fn update(&mut self, title: &MemoTitle, content: MemoContent) -> Result<Memo> {
        ...
        if !file_path.exists() {
            return Err(MemorandaError::MemoNotFound { title: title.to_string() });
        }
...
mod tests {
    ...
    async fn test_memo_storage_create_and_get() {
        ...
        let created_memo = storage.create(title.clone(), content.clone()).await.unwrap();
    ...
    async fn test_memo_storage_update() {
        ...
        storage.create(title.clone(), original_content).await.unwrap();
        
        let updated_memo = storage.update(&title, updated_content.clone()).await.unwrap();
    ...
    async fn test_memo_storage_list() {
        ...
        storage.create(title1.clone(), content.clone()).await.unwrap();
        storage.create(title2.clone(), content.clone()).await.unwrap();

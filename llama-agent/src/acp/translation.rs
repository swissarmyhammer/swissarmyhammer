...
/// }
/// ```
/// Required capability for a tool operation
#[derive(Debug, Clone, PartialEq)]
enum RequiredCapability {
    /// Tool requires fs.read_text_file capability
    FileRead,
    /// Tool requires fs.write_text_file capability
    FileWrite,
    /// Tool requires terminal capability
    Terminal,
    /// Tool doesn't require any specific capability
    None,
}

/// Determine which client capability is required for a tool
///
/// This function analyzes the tool name to determine which ACP client capability
/// must be available for the tool to execute successfully.
///
/// # Arguments
/// * `tool_name` - The name of the tool to classify
///
/// # Returns
/// * `RequiredCapability` - The capability required for this tool
///
/// # Classification Rules
/// - Tools with "read", "get", "list", "show", "view", "load", "glob" → FileRead
/// - Tools with "write", "create", "update", "edit", "modify" → FileWrite
/// - Tools with "execute", "shell", "terminal", "run", "bash" → Terminal
/// - Everything else → None
///
/// # Examples
/// ```
/// use llama_agent::acp::translation::get_required_capability;
///
/// assert_eq!(get_required_capability("fs_read"), RequiredCapability::FileRead);
/// assert_eq!(get_required_capability("fs_write"), RequiredCapability::FileWrite);
/// assert_eq!(get_required_capability("shell_execute"), RequiredCapability::Terminal);
/// assert_eq!(get_required_capability("http_get"), RequiredCapability::None);
/// ```
fn get_required_capability(tool_name: &str) -> RequiredCapability {
    ...
    // Terminal operations - check early as they're most critical
    if name_lower.contains("execute")
        || name_lower.contains("shell")
        || name_lower.contains("terminal")
        || name_lower.contains("run")
        || name_lower.contains("bash")
    {
        return RequiredCapability::Terminal;
    }
    ...
    {
        return RequiredCapability::FileWrite;
    }

    // Check for "rm" only as a complete word
    if name_lower
        .split(|c: char| !c.is_alphanumeric())
        .any(|word| word == "rm")
    {
    ...
    {
        return RequiredCapability::FileRead;
    }
...
/// Check if the client has the required capability for a tool
///
/// This function verifies that the client's advertised capabilities include
/// the specific capability needed for the tool operation.
///
/// # Arguments
/// * `tool_name` - The name of the tool to check
/// * `client_capabilities` - The client capabilities from the initialize request
///
/// # Returns
/// * `Ok(())` - Client has the required capability
/// * `Err(String)` - Client lacks the required capability (error message)
///
/// # Examples
/// ```ignore
/// use agent_client_protocol::ClientCapabilities;
/// use llama_agent::acp::translation::check_client_capability;
///
/// let capabilities = ClientCapabilities::default();
/// check_client_capability("fs_read", &capabilities)?;
/// ```
fn check_client_capability(
    tool_name: &str,
    client_capabilities: &agent_client_protocol::ClientCapabilities,
) -> Result<(), String> {
    let required = get_required_capability(tool_name);

    match required {
        RequiredCapability::FileRead => {
            if !client_capabilities.fs.read_text_file {
                return Err(format!(
                    "Operation requires client capability 'fs.read_text_file' which is not available. Tool '{}' cannot be executed.",
                    tool_name
                ));
            }
        }
        RequiredCapability::FileWrite => {
            if !client_capabilities.fs.write_text_file {
                return Err(format!(
                    "Operation requires client capability 'fs.write_text_file' which is not available. Tool '{}' cannot be executed.",
                    tool_name
                ));
            }
        }
        RequiredCapability::Terminal => {
            if !client_capabilities.terminal {
                return Err(format!(
                    "Operation requires client capability 'terminal' which is not available. Tool '{}' cannot be executed.",
                    tool_name
                ));
            }
        }
        RequiredCapability::None => {
            // No specific capability required, always allowed
        }
    }
...
async fn execute_tool_call(
    ...
    // Check if client has required capability for this tool
    if let Err(capability_error) =
        check_client_capability(&tool_call.name, &session.client_capabilities)
    {
        warn!(
            "Tool call '{}' blocked due to missing capability: {}",
            tool_call.name, capability_error
        );

        // Return an error ToolResult (not an exception)
        // This allows the agent to continue and handle the error gracefully
...
mod tests {
    ...
    #[test]
    fn test_get_required_capability_file_read() {
        assert_eq!(
            get_required_capability("fs_read"),
            RequiredCapability::FileRead
        );
        assert_eq!(
            get_required_capability("get_file"),
            RequiredCapability::FileRead
        );
        assert_eq!(
            get_required_capability("list_files"),
            RequiredCapability::FileRead
        );
        assert_eq!(
            get_required_capability("show_content"),
            RequiredCapability::FileRead
        );
        assert_eq!(
            get_required_capability("files_glob"),
            RequiredCapability::FileRead
        );
        assert_eq!(
            get_required_capability("search_files"),
            RequiredCapability::FileRead
        );
        assert_eq!(
            get_required_capability("files_grep"),
            RequiredCapability::FileRead
        );
    }

    #[test]
    fn test_get_required_capability_file_write() {
        assert_eq!(
            get_required_capability("fs_write"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("create_file"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("update_file"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("edit_file"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("delete_file"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("remove_file"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("file_rm"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("move_file"),
            RequiredCapability::FileWrite
        );
        assert_eq!(
            get_required_capability("rename_file"),
            RequiredCapability::FileWrite
        );
    }

    #[test]
    fn test_get_required_capability_terminal() {
        assert_eq!(
            get_required_capability("shell_execute"),
            RequiredCapability::Terminal
        );
        assert_eq!(
            get_required_capability("execute_command"),
            RequiredCapability::Terminal
        );
        assert_eq!(
            get_required_capability("terminal_run"),
            RequiredCapability::Terminal
        );
        assert_eq!(
            get_required_capability("run_script"),
            RequiredCapability::Terminal
        );
        assert_eq!(
            get_required_capability("bash_command"),
            RequiredCapability::Terminal
        );
    }

    #[test]
    fn test_get_required_capability_none() {
        assert_eq!(
            get_required_capability("http_get"),
            RequiredCapability::None
        );
        assert_eq!(
            get_required_capability("web_fetch"),
            RequiredCapability::None
        );
        assert_eq!(get_required_capability("think"), RequiredCapability::None);
        assert_eq!(
            get_required_capability("custom_tool"),
            RequiredCapability::None
        );
    }

    #[test]
    fn test_get_required_capability_rm_word_boundary() {
        // "rm" as a complete word should require FileWrite
        assert_eq!(get_required_capability("rm"), RequiredCapability::FileWrite);
        assert_eq!(
            get_required_capability("file_rm"),
            RequiredCapability::FileWrite
        );

        // "rm" as part of a word should not require FileWrite
        assert_eq!(
            get_required_capability("swissarmyhammer"),
            RequiredCapability::None
        );
        assert_eq!(get_required_capability("format"), RequiredCapability::None);
        assert_eq!(
            get_required_capability("transform"),
            RequiredCapability::None
        );
    }

    #[test]
    fn test_get_required_capability_case_insensitive() {
        assert_eq!(
            get_required_capability("FS_READ"),
            RequiredCapability::FileRead
        );
    ...
    #[test]
    fn test_check_client_capability_with_all_capabilities() {
        ...
        assert!(check_client_capability("fs_read", &capabilities).is_ok());
        assert!(check_client_capability("fs_write", &capabilities).is_ok());
        assert!(check_client_capability("shell_execute", &capabilities).is_ok());
        assert!(check_client_capability("http_get", &capabilities).is_ok());
    }

    #[test]
    fn test_check_client_capability_missing_file_read() {
        use agent_client_protocol::{ClientCapabilities, FileSystemCapability};

        // Create capabilities without read_text_file
        let capabilities = ClientCapabilities::new()
            .fs(FileSystemCapability::new().write_text_file(true))
            .terminal(false);

        // File read tools should be blocked
        let result = check_client_capability("fs_read", &capabilities);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("fs.read_text_file"));

        // File write tools should work
        assert!(check_client_capability("fs_write", &capabilities).is_ok());
    }

    #[test]
    fn test_check_client_capability_missing_file_write() {
        use agent_client_protocol::{ClientCapabilities, FileSystemCapability};

        // Create capabilities without write_text_file
        let capabilities = ClientCapabilities::new()
            .fs(FileSystemCapability::new().read_text_file(true))
            .terminal(false);

        // File write tools should be blocked
        let result = check_client_capability("fs_write", &capabilities);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("fs.write_text_file"));

        // File read tools should work
        assert!(check_client_capability("fs_read", &capabilities).is_ok());
    }

    #[test]
    fn test_check_client_capability_missing_terminal() {
        use agent_client_protocol::{ClientCapabilities, FileSystemCapability};

        // Create capabilities without terminal
        let capabilities = ClientCapabilities::new()
            .fs(FileSystemCapability::new()
                .read_text_file(true)
                .write_text_file(true))
        ...
        // Terminal tools should be blocked
        let result = check_client_capability("shell_execute", &capabilities);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("terminal"));

        // File tools should work
        assert!(check_client_capability("fs_read", &capabilities).is_ok());
        assert!(check_client_capability("fs_write", &capabilities).is_ok());
    }

    #[test]
    fn test_check_client_capability_no_capabilities() {
        use agent_client_protocol::ClientCapabilities;

        // Create capabilities with nothing enabled (using default which sets all to false)
        let capabilities = ClientCapabilities::new();

        // All file and terminal operations should be blocked
        assert!(check_client_capability("fs_read", &capabilities).is_err());
        assert!(check_client_capability("fs_write", &capabilities).is_err());
        assert!(check_client_capability("shell_execute", &capabilities).is_err());

        // Tools that don't require specific capabilities should work
        assert!(check_client_capability("http_get", &capabilities).is_ok());
        assert!(check_client_capability("think", &capabilities).is_ok());
    }

    #[test]
    fn test_check_client_capability_error_messages() {
        use agent_client_protocol::ClientCapabilities;

        let capabilities = ClientCapabilities::new();

        // Check that error messages include tool name and capability
        let read_error = check_client_capability("fs_read", &capabilities).unwrap_err();
        assert!(read_error.contains("fs_read"));
        assert!(read_error.contains("fs.read_text_file"));

        let write_error = check_client_capability("fs_write", &capabilities).unwrap_err();
        assert!(write_error.contains("fs_write"));
        assert!(write_error.contains("fs.write_text_file"));

        let terminal_error = check_client_capability("shell_execute", &capabilities).unwrap_err();
        assert!(terminal_error.contains("shell_execute"));
        assert!(terminal_error.contains("terminal"));
    }
}
